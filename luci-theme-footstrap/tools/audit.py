#!/usr/bin/env python3
"""Static audit of the luci-theme-footstrap CSS sources (styles/, one dir per cascade layer;
cascade.css is generated by build-css.sh and not read here — docs/css.md).
...
screenshots do not answer it, is docs/css.md "Proving a CSS change".
"""
...
# removed; see 02-tokens.css). --fs-bar-live is written on :root by fs-chrome.js: the bar's MEASURED
# height, which a token cannot be, because the bar grows when the brand wraps or the menu takes a row
# of its own. Every read carries `var(--fs-bar-h)` as the fallback, so a frame rendered before the
# chrome is fitted still sticks to the designed height.
VAR_ALLOW = {"--zone-color-rgb", "--fs-bar-live"}

def _strip_for_balance(s):
    """Drop comments and quoted strings before counting brackets.

    Counting raw punishes dense comments: one explaining a hazard ("`(` IS on the allow-list")
    carries a lone paren and the file reads as unbalanced. This is only a smoke test for a truncated
    file, which stays obvious once comments and strings are gone.
    """
    out, i, n = [], 0, len(s)
    while i < n:
        c = s[i]
...
def audit_vars(s):
    # Comments first: a comment quotes the code it explains — 02-tokens.css spells the removed
    # bridge out as `rgba(var(--x), .3)` — and scanning raw turns that prose into a phantom
...
def _strip_comments(src):
    ...
    rules() derives every `file:line` it reports from this stripped copy, so dropping a comment with
    its newlines shifts every reported line up and the finding gets "fixed" in the wrong rule.
    Newlines are whitespace to the parser, so substituting them in place costs nothing.
    """
    out, i = [], 0
    while i < len(src):
        j = src.find("/*", i)
...
def audit_base_dead(css):
    ...
    theme/page beat base regardless of specificity, so these are unreachable — but deletability is
    decided by the whole selector GROUP, not the one selector. Base groups upstream widgets together
    and theme often repaints only the first: dead for it, alive for the rest. Deleting the rule
    because it "looks overridden" un-themes widgets no shipped LuCI page renders but a third-party
    luci-app-* does (the coverage contract, docs/conventions.md). So only an all-repainted group is
    reported as removable; a mixed one is backlog, and absorbing it means writing the uncovered
    selectors into `theme` first, never deleting here. Matching is conservative (identical selectors
...
def report(title, lines, empty="none", *hints):
    ...
    drifted. The real hazard is the count: a new section whose author forgets `findings +=` is
    silently non-gating under --strict, which is the one thing this script is for. Returning the
    number makes forgetting it impossible.
    """
    print(f"\n== {title} ==")
    if not lines:
        print(f"  {empty}")
...
def main():
    ...
    # Not a report() section: it re-groups by selector rather than listing findings, and is
    ...
    # A plain run is a report and always exits 0; --strict fails the build on anything reported.
