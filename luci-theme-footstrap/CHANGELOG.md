...
- **The four crew roles answer in caveman full, the mode the lead runs in.** A subagent inherits no SessionStart hook, so `/caveman full` reached the lead and stopped there: the four role prompts kept returning full prose into a thread that was itself compressed, and the lead quotes those blocks verbatim. Each role file carries its own "Voice" section now, covering every line the role emits and not the block alone — a question back to the lead, a `BLOCKED` line, a note on a gate that did not run. Compress the prose around the data, keep commands, flags, paths, numbers, quoted errors and every negation exact, and never add a word to sound caveman. What a role writes to disk is untouched by this: code, comments, a changelog line and a handoff stay normal English prose. `docs/crew.md` and the delegation contract in `CLAUDE.md` say the same, so a hand-written delegation carries it too. `docs/development.md` gains the host trap this session hit on the way: `npm test` fails `resolveUnderRoot` on Windows and 154 of 154 pass in WSL, because `path.resolve` is `path.win32.resolve` there — the suite is re-run under WSL before a red case is read as a defect.
...
- **`.cbi-checkbox`'s toggle switch paints on the `<input>` itself instead of on `label[for]`, so it survives an app that hides that label.** forum.openwrt.org/t/251930 post #141: `luci-app-modemdata` injects an unlayered `label[for] { display: none }` flagged important, scoped to its own markup and written expecting the switch to live on the native input as it does under bootstrap — and the theme's input was the one already hidden (`opacity:0; width/height:0`), so the whole control vanished. The pill, knob, `:checked`/`:focus-visible`/`:disabled` states and the WCAG 2.5.8 hit-area overlay all move to `input[type="checkbox"]`; `label[for]` takes over the input's old invisible placeholder (`position:absolute; opacity:0; width/height:0`) so the flex row's gap to a following caption is unchanged. New `docs/gallery.html` fixture reproduces the report's CSS shape (`label[for]{display:none!important}` scoped to itself): measured in a real Chromium page, the input computes 40x22px, `opacity:1`, background painted (`rgb(86,157,245)` checked / `rgb(55,62,71)` unchecked) in both states with `label[for]` at 0x0/`display:none`, against 0x0/`opacity:0` on the input before this fix. A stock checkbox (no hostile CSS) is unchanged: same 40x22px input, same colours, keyboard `Tab` still shows the `-solo` focus ring, a plain mouse click shows none, and the `tooltip`-labelled fixture still paints no second switch on the hint-icon label.

- **A framed `.fs-dt` table keeps its rounded top corners under any header shape, and an app's `position:relative; top` on a cell no longer steps the row separator.** Same report, forum.openwrt.org/t/251930 post #141, luci-mod-dashboard's "Info panel" at 1320px content width. The corner-rounding rule keyed only on `.tr.table-titles`, which `fs-select.js` never writes onto a real `<thead>`'s header — the DHCP table's `<thead>` holds its `<th>`s directly (E()-built, no `<tr>`), the Wi-Fi table nests a real `<tr>` inside `<thead>` that never gets `.table-titles` either — so both dashboard tables kept a square top-left/top-right `.th` (0px radius) against the frame's 12px, with the square header background painting past the arc. `30-tables.css`'s corner selectors now also match `:is(.tr.table-titles, .thead) .th:first-child`/`:last-child`, closing both shapes. Separately, `.Dashboard … .devices-info .tr .td:nth-child(3) { position:relative; top:-3px }` shifted that cell 3px off its neighbours (y=892.7 vs 895.7) and dragged its border-bottom — footstrap's row separator and striping are per-cell, unlike bootstrap's `border-collapse: collapse` rows, so the step shows only here. `:is(#view, #modal_overlay) .fs-dt .td { top: auto; bottom: auto }` cancels it: the app's re-hosted `<style>` lands in the same `theme` layer (`fs-sheets.js`), so the one id plus two classes outranks its seven classes on specificity alone, no `!important` — the physical pair, not the `inset-block` shorthand, since the sheet has never used a logical inset property before and `css-floor` has no support decision for one. Three new `docs/gallery.html` fixtures (bare `<th>` under `<thead>`, `<tr>` inside `<thead>`, and a mocked offset cell under `#modal_overlay`) give `table-contract`/`computed-diff` something to measure without a router.

- **`install.sh` decides "the feed is configured" from the exact, active repository line it itself writes, places that line FIRST in customfeeds.list/customfeeds.conf instead of appending it, disables (never deletes) every other active line naming the feed's host or name, and asks the configured repository — not the installed database — before saying "current" or naming a solver decision.** forum.openwrt.org/t/251930 posts #160-162: a 25.12/apk router's `customfeeds.list` held a line the old `grep -q "$FEED_HOST"` accepted but apk never read, so re-running printed "already configured", then "Already current … the feed carries nothing newer" while owfeed-packages had carried 0.14.13-r1 for 7 h and the router stayed on 0.14.10. Measured on real apk: its repository-file reader silently stops at the first line it cannot parse, so an admin's own unrelated line (indented, a bare word, an opkg-style `src/gz` line dropped in the apk file) sitting above where the feed line used to be APPENDED could cut the feed off with no error at all — worse, a router this project's own OLDER installer had already configured has the exact line sitting after exactly that kind of admin line, so "is it present" alone stayed true forever while apk never reached it. The line is now MOVED to be the first non-comment line whenever it exists anywhere else, not only written there when absent (the closing message says which: "Adding"/"Feed added" only when it was missing, "Moving … to the top" when it merely had to be repositioned, "already configured" when nothing changed), and every other active line naming our host (case-insensitive) or, on opkg, our src name, is commented out and reported by name; a line for any other host is left alone. Neither this nor the stale-line cleanup ever `mv`s a replacement over the customfeeds path directly — both resolve it with `readlink -f`, copy its mode and owner onto a temp file in the same directory (`cp -p`), write the new content into THAT copy, and only then `mv -f` it over the resolved target: one atomic rename, on the same filesystem, that keeps a symlink a symlink and leaves the original untouched if anything before the rename fails. A failed write is never reported as a success either: the "disabled:" lines wait until the rename actually lands, and the outcome is a real exit status the caller checks with `if`, not `$(...)`, whose own failure `set -e` cannot see through — a failure to write the feeds file now exits 1 with one clear error naming the file, leaves it byte-identical, and removes every temp this run created, instead of printing "Adding the feed…"/"Feed added"/"disabled: …" over a write that never happened. `feed_offer()` asks what the repository itself offers — apk's per-version source list from `apk policy` compared against the exact configured URL, opkg's own downloaded list file under `/var/opkg-lists/$FEED_NAME` — instead of `apk list`/`opkg list`, which both keep answering for the installed package once its repository is gone entirely; the closing message chooses between "current", naming the config file to check when the repository offers nothing at all, and the package manager's own pin/hold decision (`apk policy`/`opkg list` shown) when it offers more than what is installed, with no separate guessed "feed hasn't caught up yet" wording layered on top. Reproduced end to end on `owrt2410`/`owrt2512` and their twins with 0.14.12 installed and 0.14.13 in the feed: fresh install ("Adding"/"Feed added"), an older-installer-configured router with an admin line above the exact line ("Moving … to the top", upgraded), an unparsable admin line left untouched, another active line for our host (other branch, CRLF, uppercase), a customfeeds file that is itself a symlink to a mode-600 target (link and mode both survive), three/four runs leaving one active line and settling on "already configured", `opkg flag hold` / an apk `--virtual` pin, the feed host blocked (falls back to the signed release, no guessed wording), an untrusted key refused, and `sysupgrade -l` naming both the customfeeds file and the apk key's `keep.d` entry.

- **The header "Refreshing" pill stays clickable after every SPA navigation, and after a page empties its own poll queue with no navigation at all.** forum.openwrt.org/t/251930 post #141, reproduced on owrt2410/24.10.8 opkg, Chromium, Polish UI: after one navigation between two polling pages the pill read "Odświeżanie" with no `data-clickable` and did nothing on click, 2 of 3 runs. `fs-router.js`'s teardown flushed the poll queue and called `L.Poll.stop()`, which dispatches `poll-stop` synchronously to two listeners — luci.js's own (`showIndicator('poll-status', 'Paused', null, 'inactive')`, always with `handler: null`) and this theme's (`hideIndicator('poll-status')` on an empty queue) — and which ran first decided the outcome: luci.js registers from `setupDOM()`, reached through an async chain, this module at eval from the inline `L.require('menu-footstrap')` in `partials/footer.ut`, so network/cache timing picked the order. Run backwards, the theme's hide removed the span before luci.js recreated it for "Paused" with no click handler — `ui.showIndicator()` binds one only when it first builds the element — and the next `poll-start` found that span already there and only changed its text, forever. A first fix replaced the listener with a direct call right after `L.Poll.stop()` in the navigation teardown, which covered that path but silently missed the other caller that reaches the same `stop()`: a VIEW emptying its own queue with `L.Poll.remove()` and no navigation (`luci-mod-status`'s Realtime Graphs on unload, `luci-app-banip`'s log template) — reproduced live, owrt2410b and owrt2512b, add-then-remove on Statistics: `{"active":false,"q":0,"pill":{"text":"Paused","clickable":true}}`, unchanged by a click. The listener is back, covering both callers, but its hide is now deferred one `queueMicrotask()` past the synchronous `poll-stop` dispatch instead of running inline: a microtask runs only once the whole synchronous turn that dispatched the event — every `poll-stop` listener, in whichever order — has unwound, so the check always reads the state stock's listener actually left rather than racing it, and reads the queue at that later moment rather than at dispatch time, so a queue the same navigation's incoming page has already refilled by then is left alone. `tests/poll-status.test.mjs` drives both callers (a navigation's queue flush and a bare `Poll.remove()` to empty) in both listener orders, plus a direct check that the hide does not fire inside the synchronous dispatch itself — 13 cases, all passing; a manual pause and a `Poll.remove()` that leaves one poller queued are both left alone. `docs/spa-router.md`, "Teardown".
...
- **Everything CI runs can now be run here, and the stands stopped being the bottleneck.** `tools/ci-local.sh` reproduces every job and slice of `.github/workflows/build.yml` — including `jsmin-verify`, the owfeed build, the install-on-a-real-router leg and the per-engine anchor matrix, none of which any local command covered — with the same push/pull-request split, and it prints what it could NOT reproduce (signing, artifact hand-off between jobs) rather than passing silently over it. `owlab.yaml` gains a second instance of each OpenWrt line, so concurrent measurements stop colliding and nobody has to answer a question about the theme on an ImmortalWrt box because the others were busy.

- **`npm run pseudo-loc`, a gate that renders `docs/gallery.html` with every string 45% longer and fails on overflow — every RU layout fault landed above shipped because nothing rendered anything but English. Now wired into `npm run check:slow`, its 117 first-run findings triaged to 0.** Each word grows by a fixed amount, filler drawn from its own letters (no new break point, matching how a real translation lengthens a word rather than a sentence), Latin letters swap for accented look-alikes, and three shapes of real layout are asserted at 320/390px across all three densities: an element's own content past its box, a normal-flow box past its parent's edge, and content clipped by a scroller-less ancestor. Every check runs TWICE — once in English, once pseudo-localised — and only a finding that GREW past `--eps` between the two is reported, or the gate re-raises overflow already present in plain English (13 such findings measured on this tree: a synthetic unbroken RU compound, a breadcrumb path, an iface stat line — real, but not caused by translation and not this gate's to report). Three more false-positive shapes are guarded structurally: an empty `.modal` LuCI keeps in the DOM at all times, a collapsed submenu's links (an ordinary `display` under a `display: none` ancestor still gives zero client rects), and a deliberately fixed foreign width (the filemanager's 600px case, already decided in `tools/baselines/live-audit.json`) — a plain selector ALLOW list, the same shape as `a11y-gallery.mjs`'s `.exclude()`. Proven to bite: `flex-shrink: 0` on `.cbi-checkbox > label[for]` (task 0145, above) reverted in a scratch copy outside this checkout turns 6 clean width/density points into 6 findings, every one naming `label[for]` itself. The 117 findings left untriaged at first landing (9 distinct root elements, the rest the same element at another width/density) split into two piles, `--eps` untouched either way: five are the dropdown-ellipsis shape (task 0149) by other selectors — a breadcrumb path, a tab-strip label, a filename column, all shrink-then-ellipsis correctly and merely *measure* self-overflow the way any truncated element does — plus the meter's already-measured, margin-tolerant reserve (marginal even in English; pseudo-loc's own bracket wrapper, not a translation, tips it over) and the gallery's own deliberately-unbreakable `#g-longlabel` torture compound; all six are ALLOW-listed in `tools/pseudo-loc.mjs` with the reasoning inline, `--eps` untouched. The other three were real: fixed below (`.ifacebox-body`, `.cbi-filebrowser > .right`, `.cbi-checkbox`) and in `docs/gallery.html` (three `<input>` demo captions that read as markup, not a real button's label). Verification (task 0161) found the ALLOW walk excusing more than the selector it matched: it climbed from every match to `<body>`, excusing every ancestor along the way and reading 311 of 822 elements and 14 of 22 gallery sections as unmeasurable — including all four buttons of the "Row actions, plain table (Startup-style)" fixture this same triage had just added, so a reverted `flex-basis: auto` (`theme/55-buttons.css`, below) went undetected. Fixed to match only the elements each selector names; re-running then surfaced 66 findings, every one a cascading duplicate of the same three already-accepted leaves (`#g-longlabel`, `.cbi-progressbar`, `.table`) pushing their own CONTAINER's `scrollWidth` over (cumulative, not a new fault) — each container is now named explicitly with `:has()` against the leaf that causes it, and `.g-wrap` (the gallery's own outermost wrapper, not a widget any page renders) is excused outright, since it reads as "widest content on the page" for as long as any of the three exists anywhere on it. `.table *`'s own wildcard separately swallowed every row-action BUTTON inside a `.td.cbi-section-actions` cell, unrelated to the no-JS-carding contract that entry exists for; narrowed with `:not(.cbi-section-actions, .cbi-section-actions *)`, the Startup fixture's buttons now measure clean, and reverting `flex-basis: auto` to check reproduces exactly the expected finding (6 points, `button.cbi-button` self-overflow on "Принудительно завершить"). Separately, the SELF check's `eps + margin` tolerance could go NEGATIVE under a negative margin (`.zonebadge .cbi-tooltip`, `base/90-widgets.css`: `margin: -1.6em 0 0 -5px`, an overlap trick, not reserved room), which widens the pass condition instead of narrowing it and can report `by` as low as 0px — clamped with `Math.max(0, …)` so a negative margin now costs nothing, same as carrying none.

- **A real page, `admin/system/footstrap` titled Footstrap, dispatched by the menu like any other page, and the tab this theme used to staple onto `luci-mod-system`'s form is gone.** `root/usr/share/luci/menu.d/luci-theme-footstrap.json` (a menu.d the package shipped none of before this) and `htdocs/luci-static/resources/view/footstrap/appearance.js` add an ordinary `view`; the panel itself is unchanged — `fs-appearance.js`'s `build()` is reachable through the one export left, `renderStandalone()`. Motivated by a tab lost after Save & Apply on a real router, never reproduced on either stand: a panel stapled onto another app's form needed four compensating mechanisms a dispatched page needs none of, and all four are gone from `fs-appearance.js` now rather than staying beside the page that replaced their reason to exist — the `MutationObserver` on `#maincontent`, the 5 s mount deadline and its `console.error` path, the `body[data-page]` tracker (`PAGE`, `onPage()`, `_routeObserver`), the stale-group `WeakSet` (`disqualifyCurrentGroups()`), the `sessionStorage` return-key dance (`armReturn()`/`takeReturn()`), and `mount()`'s own `<li>` injection into `.cbi-tabmenu` — along with the `standalone` flag `build()` carried for exactly this fork, now that there is one caller and one behaviour. `menu-footstrap-common.js`'s page-module map drops `fs-appearance` the same way: a page reached by `menu.d` is loaded by the dispatcher, not by watching `body[data-page]` for a page the theme does not own. This also closes a latent bug never reproduced on either stand: `disqualifyCurrentGroups()` read `#view` at `data-page`-change time on the assumption that it still held the OUTGOING page, while `fs-router.js`'s navigate() deliberately inserts the incoming staged view first in tree order — only microtask ordering kept the two from colliding, and a second `data-page` write landing while a stage held an already-initialised group would have blacklisted that group and left the tab unmountable for the life of the document, the most plausible account of a user report that could never be reproduced. Minified `fs-appearance.js` drops from 15,218 B to 12,950 B (before the shared-name mangle pass); `resourcesJs` — 94,960 B, 960 B over its 94,000 B budget once the page work and the SVG sanitizer above had both landed — is back under it, at 92,690 B, with no change to the budget itself. No ACL change: `luci-theme-footstrap`'s existing `read`/`write` grant on `uci footstrap` already satisfies `depends.acl`'s "read or write" test, so the new menu entry gates on the same group the Save-to-router button already used. The view leaves `handleSave`/`handleSaveApply`/`handleReset` set to `null`: a view that leaves them unset still gets `view.js`'s default footer — measured live, `/admin/system/footstrap` rendered a working `Save & Apply | Apply unchecked | Save | Reset` bar wired to `apply_rollback`, and clicking it opened a modal reporting no changes to apply, since every axis here is a client-side preference already applied, not uci form state staged for a commit.

- **`docs/conventions.md` states the flex item's automatic-minimum-size rule, the one CSS behaviour four unrelated-looking bugs this session turned out to share.** A flex item's automatic minimum defaults to its content size unless the item's own `overflow` is not `visible`, which zeroes it — and each direction of that rule shipped a bug: the toggle knob left its pill (no `flex-shrink: 0`, so the pill shrank below its declared width while the knob did not), a form label escaped its section on ssclash (`flex: 1 1 100%` with no `min-width`), a closed dropdown's `overflow: hidden; text-overflow: ellipsis` sat inert (the automatic minimum held the `li` at content width, so nothing was ever offered to clip), and a row-action button's `overflow: hidden` zeroed its own minimum and made `flex-grow` divide the row by count instead of by need. No gate holds the rule in general — `pseudo-loc` catches the first shape once a pseudo-localised caption grows past a shrunk box, and nothing catches the third, an inert clip rule that never overflows anything.

- **`live-audit` gains `--lang`, so a signature that only reproduces in Russian gets its own baseline entry instead of failing every English run or hiding behind it.** Switching a stand to Russian by hand and running the unmodified gate produced 77 (owrt2512) / 70 (owrt2410) findings — not regressions, RU text is measurably longer and the checks that read box geometry are exactly the ones length moves — which would have either failed every English CI run or, if length-only findings were dropped instead, hidden the two real faults fixed below. `--lang ru` sets `luci.main.lang` on each router before the sweep and restores `auto` after (even on a crash, so an unflagged run can never silently inherit it), and keys the result `<stand>@ru`, composing with `--engine` the same way `@firefox` already does — a per-language union, never a merge into the English set. `owrt2512@ru` (255) and `owrt2410@ru` (254) are now baselined; 62 / 55 of those are RU-only, and one group of them — a `TypedSection` table (`firewall/zones`, `firewall/custom`, `services/ddns`, `services/samba4`) clipping its whole header row 49-65px at 1024px because `.cbi-section-table` carries none of `.table.fs-dt`'s reflow rules — is a real, currently unfixed defect, filed rather than patched here (`docs/development.md`). Also triaged: 58 stale signatures that no longer reproduced on either stand (an app-version bump on ssclash, an arrival/resize signature dedupe, both closed by the two fixes below) are dropped from `tools/baselines/live-audit.json`, and `/admin/services/ssclash/settings|320|clipped|div.cbi-section` — held in the baseline as an ordinary finding since ssclash was added — turned out to be the app's own inline `grid-template-columns: repeat(auto-fit, minmax(320px, 1fr))`, a 320px column floor that cannot fit a 320px viewport in any language, unrelated to the RU-only form-label fix elsewhere in this file that shares its page.

- **`docs/development.md`'s stand traps gain five more from this session, each one that cost a re-run before it had a name.** A mouse click parks focus and re-seats Tab order, so a probe that clicks before pressing Tab reports the skip link unreachable; a collapsed submenu's links carry an ordinary computed `display` while the parent `ul` is `display: none`, so `getComputedStyle` alone reads them as laid out (`getClientRects().length` does not); `$?` is unreliable in this WSL bash (`false; echo $?` prints `0`), the worst of the three because it turns a red gate green with nothing in the log to say so; on System → System, `button.cbi-button-apply` selects "Скопир. из браузера", not the Apply control (`.cbi-page-actions .cbi-dropdown.cbi-button-apply li[data-value="0"]` is), which produced a false NOT REPRODUCED on a repro battery keyed on the wrong selector; and `npm`, `python3` and any `*.sh` gate cannot run from the Windows side of this checkout at all — `npm run smoke` dies with `EFTYPE`, `sh tools/check-acl.sh` hits the Python Store alias — only from WSL, with node put on `PATH` by hand.
...
- **The dev stand's Russian catalogues are pinned in `owlab.yaml`, not installed by hand.** The first Russian sweep (below) found its faults by installing 22 `luci-i18n-*-ru` packages by hand; without a pin here the next `owlab up` wipes them and the next sweep starts from zero. One entry per `luci-app-*` above that ships a translation — `luci-i18n-opkg-ru`, `-wireguard-ru` and `-openconnect-ru` are deliberately absent, published by neither feed (25.12/apk, 24.10/opkg). RU strings run longer than English's, and this theme's gates rendered English only until 2026-09-06 — this pin is what makes the gap repeatable to sweep.

- **The pattern upload sanitizes an SVG instead of refusing it, when sanitizing is honest.** `_svgObjection()` (fs-assets.js) refused the whole file over a `<script>` element, an `on*` handler, a `javascript:`/off-router `href`, or a `style` value reaching off the document; `_sanitizeSvg()` walks the same parsed tree ONCE and now removes each of those instead — the element, the attribute, or (for `style`) just the one bad declaration, keeping the rest of the value — and hands back the ORIGINAL bytes untouched when nothing needed to go, so a clean upload is never run through a re-serializing round-trip that can silently rewrite namespaces and whitespace on a 138 KB tile. Cleaning is not the security boundary and was never meant to be one: the rpcd ACL authorises the upload regardless of what this file decides, and `root/www/cgi-bin/luci-theme-footstrap-pattern`'s `Content-Security-Policy: default-src 'none'; sandbox` is what actually stops the file running if it is ever opened directly — so this only trades a refusal an admin had to go hand-edit their way around for a working tile and a `ui.addNotification()` banner naming what came out ("removed N unsafe element(s) and N external reference(s)"). Only four things still refuse the file outright: not an SVG, a parser error, the wrong namespace, or nothing left once the unsafe parts are gone. Costs 1,148 B minified (4,630 B -> 5,778 B) — on the Appearance page module only, so the cold-page budget does not move.

### Fixed

- **Three places still described Appearance as a tab and gave, as the reason, a claim the shipped page now contradicts; a fourth carried the same contradiction about `fs-overview`; a fifth size-budget comment was one commit stale.** `view/footstrap/appearance.js`'s header comment said "the tab stays for now" and called `renderStandalone()` "the exact panel the System -> System tab mounts" — the card that removed the tab had already run, and the tab-mounting machinery is gone from `fs-appearance.js`. `docs/design-system.md` (twice) and `header.ut`'s right-cluster comment gave "a theme owns no dispatcher node of its own" as why Appearance was a tab, contradicted by `docs/architecture.md`'s own next section, which already describes `fs-appearance` owning `/admin/system/footstrap`. That same `architecture.md` carried the identical claim for `fs-overview` two paragraphs earlier: corrected to the real boundary — a theme MAY register a dispatcher node, `fs-overview` still watches `body[data-page]` only because Status -> Overview is `luci-mod-status`'s own page, not because a route is out of reach. `tools/size-budget.mjs`'s newest `cascadeCss` comment recorded 127,001 B / 499 B of head-room; a commit landed after it was written and the sheet now measures 127,100 B / 400 B — the two figures are corrected, the reasoning they were attached to is not. `docs/development.md` gains a stand trap from the same audit: a leftover `dist/` in the checkout carried a stamp (`0.14.11-r1`) its own bytes did not match, building 470/463 B (CSS/JS) heavier than a clean `git archive <tag>` build of the same tag — comparing against it would have read real growth as a shrink.

- **The toggle switch's knob no longer sits outside its own pill when a caption follows it.** `.cbi-checkbox` is a flex row, and a caption after the switch — `label.cbi-checkbox > input + label[for] + TEXT`, package-manager.js's and banip's real markup, not a bare switch with nothing beside it — made the pill itself a second, shrinkable flex item: the knob's `::after` keeps its literal size and a travel computed from the DECLARED width, so a shrunk pill left the knob past its own right edge. Measured with a Russian caption, the one long enough to break it at default density on every phone width, on the live Install dialog on both stands: RU/Normal/390px went from a 30.17px used pill (40px declared) and a 6.83px knob overhang to -3.00px; English only broke it at Density=Large — 36.22px used against 46 declared and a 6.47px overhang, to -3.31px. `flex-shrink: 0` on `.cbi-checkbox > label[for]` is the whole fix; a second guard was tried and measured redundant. The gallery's switch specimen carried no caption at all, so the pill was never a second flex item there and every gallery gate — `computed-diff` included — stayed green through the bug's entire life; it now carries the real markup and the caption that broke it.

- **Three Russian-only overflow faults the first RU sweep found: an interface-box head, an adblock tab and the Save & Apply caption.** The interface-box head inherited `.ifacebox`'s `white-space: nowrap` with no override of its own — unlike its sibling `.ifacebox-body`, fixed the same way already — so LuCI's RU string for "IPv4 Upstream" ran past its own box: measured 301px in a 288px column at 320px/Density Large, 41.3px past `#view`, the `.cbi-section` clipped 42px with no scroller (EN: 0). `white-space: normal; overflow-wrap: anywhere` on `.network-status-table .ifacebox-head`, the body's own pair, is the whole fix. adblock's Overview tabs escaped their column next to it: the `<a>` already carries `text-overflow: ellipsis` (`base/50-chrome.css`), but the `<li>`'s `max-width: none` (`theme/40-tabs.css`) left its automatic flex-shrink minimum at the label's full content width, so the ellipsis never engaged — "Настройки межсетевого экрана" measured 293px in a 264px column at 320px/Large, 28.6px past it, the section clipped 30px (EN: 0). `max-width: 100%` on the plain (non-mode) `.cbi-tabmenu > li` is a no-op everywhere the tab already fit. Last, the Save & Apply split button's caption was cut worse in RU but already broken in EN: Save/Reset are plain text with nothing to give, so a `nowrap` `.cbi-page-actions` put the WHOLE shrink onto the one sibling with an ellipsis to spend, cutting up to 119px on the gallery's own EN fixture at 320px/Large. `flex-wrap: wrap` on `.cbi-page-actions` sends whichever item no longer fits to its own line instead of shrinking it, measuring 0px cut at every width/density this bug was filed against but one — a full RU sentence at Large density in a 320px column still falls 33px short even alone on its line, which is the one case left for the caption's own ellipsis to answer.

- **A closed dropdown's value ellipsises instead of cutting itself off mid-glyph.** `.cbi-dropdown > ul > li` already carried `overflow: hidden; text-overflow: ellipsis`, but a flex item's automatic minimum floors it at its own content width until `min-width` says otherwise, so the ellipsis never engaged: measured at 320px, RU cut mid-glyph 132px (Compact) / 225px (Normal) / 280px (Large) on https-dns-proxy, 66px on `nlbw/config`, 70px inside the network interface-edit modal. `min-width: 0` on the same selector is the whole fix. No `title`/`aria-label` added: the full string stays in the DOM and the accessibility tree either way — a `title` is not announced on touch, and an `aria-label` carrying the SHORTENED text would break WCAG 2.5.3 (the accessible name must contain the visible text). The mobile-only `.cbi-dropdown { min-width: 0; max-width: 100% }` override in `theme/90-responsive.css`, suspected dead against the desktop rule's higher specificity, measured NOT dead and left in place: it is the only cap on a BUTTON dropdown (`.cbi-dropdown.btn`/`.cbi-button`, e.g. Save & Apply) at phone widths, which the desktop `:not(.btn):not(.cbi-button)` rule never reaches — deleting it moved `div.cbi-dropdown.btn`'s `min-width`/`max-width` at 390px in `computed-diff`.

- **Two of three faults the second RU sweep found: a form label escaping its section, and row-action captions painting over their neighbours.** ssclash's settings form stacks a `.cbi-value-title` above its field under 767px (`flex: 1 1 100%`), and a flex item's automatic minimum size is content-based once a percentage basis is no smaller than the content — a label whose text runs short of break points renders at its own min-content instead of the row's width. Measured on the live form at 320px: 320px wide in a 254-276px row, the `.cbi-section` clipped 33-50px with no scroller to reach the rest, 0 with the English string in the same box. `min-width: 0` on `base/30-forms.css`'s `.cbi-value-title` is the whole fix — stated once on the desktop rule, it survives the later mobile layer untouched because that rule restates `flex`, never `min-width`. Separately, `base/95-luci.css` sizes every row-action button `flex: 1 1 4em` inside `.td.cbi-section-actions` (Startup, firewall rules and forwards, network, routes, ddns, processes — anywhere outside the Save & Apply footer's own `flex-wrap` fix above): a caption longer than 4em shrinks the box below its own text once the row runs short of room, and a plain `overflow: visible` button prints the rest over the next control — 14-19px on Startup, 12-13px on "Клонировать", 16-18px on "Перезапустить"/"Перезагрузить", 5px on "Принудительно завершить", 0 in English. A first fix (`overflow: hidden; text-overflow: ellipsis; white-space: nowrap` alone) clipped the spill but regressed the desktop: with a DEFINITE flex-basis, `overflow: hidden` zeroes the automatic minimum in every row regardless of whether it needed to shrink, so `flex-grow` divided the row by count instead of by need even with room to spare — Firewall Zones at 1440px went from 37.66/97.73/89.69px (v0.14.11) to 75.02/75.03/75.03px, the same 225.08px row total, 11-21px of caption ellipsised for nothing; 82 elements on owrt2512 and 108 on owrt2410 gained clipping that had none before, 27 and 60 of them at 1440px. `flex-basis: auto` on the same rule is the actual fix: it hands the automatic minimum back to the caption's own content, so `flex-grow` only ever redistributes genuine slack — reproduced on the gallery's Startup fixture at 1440px, `flex-basis: auto` reads back the exact v0.14.11 widths (101.58/137.72/112.08/230.25px), 0px moved, with `overflow: hidden` and the ellipsis left in place for the row that is genuinely narrower than its captions. That fix's own selector over-reached too (task 0151): `.td .cbi-button, .td .btn` matched every button in every cell, not just the action ones this was filed against — measured collateral, 14 buttons took `overflow: hidden` for nothing that clipped, the icon-only rail drag handle and DHCP leases' plain-cell "Reserve IP" among them, growing the leases rows 50px → 50.84px (10 rows at 1280). `.cbi-section-actions .cbi-button`/`.btn`, two selectors later in the same rule, already reaches every real action button — `.cbi-section-actions` sits on the `.td` itself — so the bare `.td` pair is dropped outright, saving 25 B; the original fault stays fixed, verified on the gallery's Firewall Zones and Startup-RU fixtures. The third finding — a closed `.cbi-dropdown`'s value clipping itself worse in RU, on https-dns-proxy, `nlbw/config` and the network interface-edit modal — reproduces (its own box already uses the full width its field is given, confirmed empirically; the shortfall is real translation length, not wasted padding) but has no fix that stays inside this task's file list without either widening the mobile field width (`theme/90-responsive.css`, unassigned) or letting the closed control wrap to two lines, which conflicts with the `height: var(--fs-control-h)` invariant `theme/65-dropdown.css` already documents and measured — left open, filed for the next card.

- **`css-floor` classifies CSS VALUES now, not just properties, and catches `overflow: clip` sailing a version past the theme's own declared floor.** The gate scanned properties, at-rules, pseudos, functions and units but never a property's VALUE, so `overflow-x: clip` (`.fs-main`) and `overflow: clip` (`.fs-staging`) — Safari 16, one version above the declared Safari 15.4 floor — passed silently. `tools/css-floor.mjs` gains a `VALUES` table, checked only for the handful of property/value pairs worth tracking (most values share their property's support, so this stays a short list rather than a scan of every value in the sheet), and classifies `overflow: clip` SOFT: measured in a real Chromium with the declaration forced to `initial` (what an unsupported value leaves behind), the desktop sidebar's `.fs-main` degrades to a sideways scrollbar (its sibling axis is already `overflow-y: auto`, and CSS Overflow 3's cross-axis correction takes `visible` to `auto`), while the top/narrow layouts and `.fs-staging` degrade to whole-page horizontal scroll — exactly how stock LuCI, with no `clip` at all, already renders — and `.fs-staging` stays `visibility: hidden` regardless, since nothing inside a staged view sets `visibility: visible` on itself. `docs/css.md`'s progressive-features table and `tools/baselines/css-features.json` both gain the entry.

- **The pattern upload's SVG check no longer refuses a `style` attribute outright, which turned away nearly every real upload.** `_svgObjection()` (fs-assets.js) treated any element carrying `style=""` as unacceptable — an Inkscape export examined for this fix carried 278 such attributes, `style="fill:#100f0d;fill-opacity:1;fill-rule:nonzero;stroke:none"` and its kind, not one of them containing a reference of any sort. The attribute is now refused only when its value reaches off this document — `url(` naming anything but a `#fragment` inside the same file, `@import`, or a CSS `expression(` — so a same-document `url(#gradient)`, how every SVG gradient and clip-path is written, is admitted, while `url(//evil.example/x.png)` and `url(http://evil.example/x.png)` still are not. The `<style>` ELEMENT stays refused whole and unconditionally: a tile has no legitimate use for one, and parsing its CSS to tell a safe declaration from a dangerous one would be exactly the grammar-guessing this check avoids everywhere else.

- **Three real faults `pseudo-loc`'s first run found among its 117 findings: a Port-status tile's stat line, the file-upload strip's button row, and a long checkbox caption.** All three trace to the same shape: a text-only `nowrap`/ellipsis rule reaching a child that has no text to ellipsis. `.ifacebox .ifacebox-body` inherited `.ifacebox`'s `white-space: nowrap` (right for a one-word head, wrong for a stat line) with no override of its own — the plain Port-tile fixture, no `port_*.svg` icon, measured 33px of self-overflow at 320px/Large in PLAIN ENGLISH, already past the fix `#cbi-network-interface` and `.network-status-table` each carried for the same rule under different scopes; the fix moves to the base `.ifacebox .ifacebox-body` rule once, and the two scoped copies drop their now-redundant `white-space`/`overflow-wrap` pair. `.cbi-filebrowser > .right` inherited the SAME text-only rule (`.cbi-filebrowser > *`) meant for a one-line row of text, not a nested flex row of buttons: `nowrap` blocked the Create/Cancel pair from ever wrapping and `overflow: hidden` clipped whichever one lost with no "…" to show for it — measured 76.42px of Cancel's label past its box at 320px/Large, no ellipsis rendered, the severe "text the reader cannot reach" case this gate's own docs name. `white-space: normal; overflow: visible` on the same selector restores the ordinary button-row wrap the theme gives everywhere else. `.cbi-checkbox`'s caption (a plain text flex item, not a torture string — the theme already ships one real 14-letter Russian word here) had no `overflow-wrap` at all, so a single word too wide for the row pushed the whole thing 24.63px past `.cbi-value-field` at 320px/Large instead of breaking; `overflow-wrap: anywhere` is the fix, same reasoning `theme/65-dropdown.css` already gives the same two properties.

- **A meter no longer colours by fill alone: "Total Available" and "Swap free" warn on a LOW reading instead of a high one, "Buffered"/"Cached" never colour, and the package-manager's own disk bar is finally annotated at all.** Forum topic #134 (2026-09-06): Status -> Overview's "Total Available" row turned yellow at 91% free — healthy, not a warning — and "Swap free" showed permanent red at 100% free, a healthy router looking worse than the reported symptom. `annotateMeter()` (`menu-footstrap-common.js`) now reads the row's own label (`_(msgid)`, matched the way `fs-overview.js`'s `ROLES` already matches a card title — verified against `modules/luci-base/po/ru/base.po`: all four msgids below live in the `base` domain loaded on EVERY admin page, not one only Status -> Overview fetches, unlike `_('Free')`, which has no entry there and comes back untranslated) and picks one of three rules: INVERTED ("Total Available", "Swap free") fires the same 80/92 split from the OTHER end of the fill (`100 - FS_METER_WARN/DANGER`, not a second pair of numbers — the same health question asked from the opposite side); NEUTRAL ("Buffered", "Cached") never colours, a full page cache being the kernel's own bookkeeping, not a resource running out; everything else — a third-party app's own meter included, which this theme cannot tell apart from the seventeen used-based bars measured live on Overview alone — keeps the plain fill-based rule, a wrong colour on the rare inverted exception costing less than going dark on the common case. Separately, `parseMeterPercent()` gains a third, start-anchored pattern for the one title shape observed LEADING with its percentage instead of trailing it — the package-manager's own disk bar, `'6% used (62.91 GiB used of 1006.85 GiB, 943.95 GiB free)'`, measured live — which used to parse to null and get no `role`, no `aria-*` and no colour at all; the pattern cannot pick a wrong number out of the same title (`^(-?\d+)%` cannot match the leading "62" of "62.91 GiB", a decimal point following those digits rather than `%`), and is tried only after both end-anchored patterns fail. Costs 192 B minified (`menu-footstrap-common.js` 4,257 B -> 4,449 B); `resourcesJs` measures 93,130 B, 870 B under its 94,000 B budget, no change to the limit itself. `po/templates/footstrap.pot` did not carry the four new msgids yet, which is what `update-po.sh --check` (`npm run i18n`) reports as a stale catalogue rather than an untranslated one — a rescan adds them, 118 strings total. `ru` and `es` get real `msgstr`s for all four, copied from `luci-base`'s own `po/<lang>/base.po` entry for the same msgid rather than invented (`Буферизовано`/`En búfer`, `Кэшировано`/`En caché`, `Свободно в подкачке`/`Intercambio libre`, `Свободно`/`Total disponible`): `lmo_translate_ctxt()` (`modules/luci-base/src/lib/lmo.c`) resolves a hash-keyed lookup against whichever archive's entry loads first for the two catalogues sharing this msgid with no msgctxt, so matching `base`'s wording is what keeps a footstrap `.lmo` that ever wins that race printing the same word `luci-mod-status` prints everywhere else on the page, not a different one only here.

- **A WireGuard public key no longer runs past its own box, and a firewall rule-jump tooltip wraps instead of refusing to.** `code` inside `.cbi-value-title`/similar had no `overflow-wrap`, so a 44-char base64 key with no space or hyphen anywhere in it — `admin/status/wireguard` — measured 64px past the content column at 320px; `overflow-wrap: anywhere` is the same escape hatch a MAC and a hostname already carry elsewhere in this sheet. `.cbi-tooltip`'s `white-space: pre` never wraps regardless of where a space sits, so a firewall rule-jump hint ("Chain POSTROUTING, Rule #1") measured 65-72px past the column at 320px, spaces and all — `white-space: pre-wrap` keeps every literal whitespace run this rule exists for while letting the browser wrap at one of them. Both found by `live-audit`'s baseline audit (above) and confirmed to reproduce exactly by reverting each fix in turn and re-running the gate.

- **A `TypedSection` table's header row no longer clips itself at 1024px, RU: `firewall/zones`, `firewall/custom`, `ddns` and `samba4`, the group `live-audit --lang ru` filed rather than fixed (task 0157/0162, above), are closed.** `.cbi-section-table` already cards under `@container fs-content (max-width: 960px)` (theme/65-dropdown.css) below that width, the same machinery a config table with inline widgets needs; the bug was never the mechanism, it was which box the query reads. `fs-content` binds to `.cbi-map`, one level above the `.cbi-section` that holds the table and spends `--fs-card-pad` (16px a side, 32px total) before the table ever sees the width, so the query's 960px never matched what the SECTION actually had left. Measured live: firewall zones' `.cbi-map` sat at 968px — clearing the un-adjusted check — while the section handed the table only 936px for six RU headers needing 1017px, clipping the row 49-65px with no scroller anywhere near it (English's shorter headers happened to fit the same 968px, which is why only RU showed it). The threshold moves to 992px (960 + 32), reading what it always meant to regardless of column count, language or viewport — not a number picked for 1024px or for Russian. A scroll fallback was considered and rejected: this table can hold `.cbi-dropdown`/`.cbi-dynlist` widgets whose open list a scroll container would clip, the exact trap `.fs-xscroll` is already kept away from. Neither `pseudo-loc` (320/390px only) nor `computed-diff` (1280/390px) would have caught this on their own — the failure lives in the 936-992px band neither renders at — so `docs/gallery.html` gains a `.cbi-map`-pinned specimen reproducing the exact 968px/1017px numbers, and `computed-diff` now reads its header row's `.th`/`.td` switching `display: table-cell` -> `block`. `live-audit --lang ru` is the gate that saw the live bug and is the one to re-run before pruning the baseline entries it left (`docs/development.md`).

- **Six live `luci-app-acme` findings a baseline rewrite dropped are back, and the RU/EN "no longer reproduce" asymmetry (95 vs 3) that rewrite's sibling run surfaced is resolved rather than papered over.** `tools/baselines/live-audit.json` lost `/admin/services/acme/logread|<width>|noname|textarea#syslog` (all six widths, `owrt2512` only) between one baseline rewrite and the next; `live-audit --all --pages-all` reproduced them deterministically on re-check — `luci-app-acme`'s own `form.TextValue` textarea, no `for=`, the same app-owned shape the rest of the `noname` group already carries — and they are restored to `owrt2512` and, since `noname` does not depend on language, to `owrt2512@ru` (not `owrt2410`, whose acme app does not render this page at all). The 95-stale-vs-3 asymmetry traces to two things: 91 signatures (46 `owrt2512@ru`, 45 `owrt2410@ru`) were the whole sub-tree of the `TypedSection` table fix already in this changelog (992px, above) — genuinely gone, live-verified, pruned — and three `owrt2410@ru` `banip/processing_log` entries need accumulated log content to overflow at all, which a freshly booted container does not have; left in place rather than pruned on one clean run's word, the same reasoning `docs/development.md` already gives the RRD-graph race. A first pass at pruning used `--prune`'s literal "replace with what this run saw" and nearly dropped the six acme signatures a second time: `noname`'s check reads `el.textContent` as a name, and this textarea's content is the log line the page's own async RPC writes in — an unresolved-content race, not a fix, indistinguishable from the RRD `img` case already on this page (`docs/development.md`).

- **The four strings `update-po.sh --check` flagged after tonight's catalogue rescan are translated, one after its English source was rewritten.** The rpcd ACL description for `luci-theme-footstrap` — shown verbatim in LuCI's own permission UI — read like a commit note ("depends.acl", "check_acl_depends", "the existing read grant already satisfies … so nothing else here changed"), not text a router admin should ever see; it is rewritten to "Footstrap theme settings saved to the router, the login background and pattern uploads, and the Footstrap settings page" and only then translated. `Footstrap` (the new menu entry's title) gets `msgstr` equal to its `msgid` in both `ru` and `es` — a product name translated is a mistake, not a missing string, noted as such beside both entries so it is not "fixed" later. The sanitizer's two upload messages (`fs-assets.js`) read fine in English as written and ship unchanged; `ru`/`es` gain real translations matching the file's existing register (`Не удалось прочитать файл.` / `No se pudo leer el archivo.` and neighbours). `npm run i18n` goes from 4 untranslated strings to 0.

- **The release runbook makes checking CI explicit and two-sided instead of "wait for a green pipeline": read `main` before tagging, read the tag's own run job-by-job after.** `v0.14.12`'s tag run failed, but the cause predated the release — the previous commit on `main` had failed the identical way the day before, on the same upstream condition (a snapshot-feed 404 taking `apk update` down under `set -eu`), and nobody had checked. The runbook (`docs/releasing.md`, `.claude/skills/release/SKILL.md`) now reads recent `main` runs before pushing (`gh run list --limit 6`) and treats a pre-existing red as something to resolve — an infrastructure condition or a real regression — before tagging onto it; after pushing, it reads the tag's own run job-by-job (`gh run view <id>`, `gh run view --job <id> --log-failed` for the failing step) and names the cause instead of only waiting for green, before the asset/`.sig` check that already existed. Also noted: `release` only runs once whatever it gates on is green, by design, so one red leg unrelated to the diff still holds the tag — and on the maintainer's Windows machine, `gh` is not on `PATH` in Git Bash or WSL and must be called by its full path, `/c/Program Files/GitHub CLI/gh.exe`.

- **`live`/`anchors` no longer install on `owrtsnap`, and a new `live-snapshot` job (`continue-on-error: true`, absent from `release`'s `needs`) carries that leg instead — a release is no longer held by somebody else's snapshot-feed rebuild.** `owlab install owrtsnap …` runs owlab's own stage-3, which opens `set -eu; apk update; …`: OpenWrt's snapshot feed rebuilds continuously and names its kmods index after the kernel hash, so a slightly older image's index can 404 mid-run with no code change involved — measured on runs 34036001267 and 34095751675, both green on `owrt2512`/`owrt2410` and red on `owrtsnap` alone, on the identical commit (`ERROR: wget: exited with error 8`, `unexpected end of file` on the kmods `.adb`, then `1 unavailable, 0 stale; 10206 distinct packages available` and a non-zero exit anyway, ahead of owlab's own per-package loop, which is already tolerant of one missing package). A retry does not help — `tools/ci-retry.sh` is for a stall, and a 404 is not one, and it had blocked a tag twice (this one and the one before it). `live-snapshot` still boots `owrtsnap`, still runs the same gates `live` runs for `owrt2512`/`owrt2410`, and still shows failed on a genuine finding for a maintainer to read; it is simply no longer in the set `release` waits on. `docs/ci.md`'s job graph and job table follow. Confirmed directly against the live feed (task 0172): `owlab up owrtsnap` reproduces the identical failure locally, `docker image inspect docker.io/openwrt/rootfs:x86_64-master` puts the base image's own build at 2026-05-29, and the feed's `kmods/` directory today lists only ten hashes, `6.18.34` through `6.18.44` — `6.18.33`, what the image is pinned to, has already rolled off that retention window entirely, so this is a standing mismatch rather than an occasional one and reproduces on every run until the upstream image is rebuilt. Nothing in `owlab.yaml` reaches it: the failing repository line is baked into `/etc/apk/repositories.d/distfeeds.list` by OpenWrt's own image build, not by anything under `defaults:`/`routers:`, and `apk update` refreshes every configured repository in one shot before the per-package loop runs, so trimming `kmod-nft-offload` (or any package) out of the list changes nothing — the command that fails never reaches that loop. The cure stays upstream in owlab; `docs/development.md` gains the trap for telling this red apart from a real snapshot regression.

### Security

- **`xml:base`, unexamined until now, no longer lets a perfectly relative `href` inside a pattern SVG resolve off-router.** Security review finding (LOW): `_sanitizeSvg()` (fs-assets.js) blocked a leading `//`, an absolute scheme and `data:`/`blob:`/`about:` on `href`/`xlink:href`, but a document carrying `<svg xml:base="http://evil.example/">` and a perfectly relative `href="x.png"` resolves against the attacker's base in a real browser — nothing in that `href` is off-router on its own, so the check had nothing to catch and the ORIGINAL bytes went through the passthrough untouched. Predates this pass (the old refusing `_svgObjection()` had the same gap) and is closed at the cause rather than the instance: `xml:base` is removed from ANY element carrying it, not only the root, because it is not itself a reference — it moves what every relative one under it resolves against, `href`, `xlink:href` and a `style` attribute's `url()` alike — so removing the one attribute that redirects resolution closes all three at once, the same reasoning `<style>` is already removed whole for above (no legitimate use in a background tile). Matched on the literal `xml:` prefix, consistent with every other attribute check here reading a qualified name rather than a resolved namespace: `xml` needs no declaration to mean this, and an export that aliases some other prefix to the XML namespace just to carry it is a narrower attack left uncaught on purpose, the same tradeoff the rest of this file already makes. Closes a second LOW finding in the same review: `tests/assets.test.mjs`'s fake `DOMParser` stand-in hardcoded every element's `namespaceURI` to SVG's regardless of any `xmlns` in the fixture, so three of `_sanitizeSvg`'s own comments claiming behaviour "measured executing on all three engines" — a prefixed `<s:script xmlns:s="…svg">`, an element re-namespaced to xhtml, a root spelled `svg` but namespaced wrong — were exercised by no test at all: every fixture read back as SVG's own namespace no matter what it declared. The stand-in now resolves `xmlns`/`xmlns:prefix` declarations down the tree the way a real parser does (`xml:` itself needs no declaration, Namespaces in XML §4), so a fixture can finally put an element in the wrong one; four new tests cover the `xml:base` case above and the three namespace claims. Costs 53 B minified in the shipped file (fs-assets.js: 5,778 B -> 5,831 B) — the namespace resolution that proves it lives only in the test file, which does not ship.

- **A CSS hex-escape hiding `url()` inside a pattern's `style` attribute no longer slips past the external-reference check.** `_externalStyleRef()` matched the literal string `url(`, so `background:u\72l(//evil.example/x)` (`\72` is 'r') reached a real CSS tokenizer as an ordinary external `url()` while the literal match never saw it — the LOW finding from the `/security-review` run on the earlier `style`-attribute narrowing (above, "The pattern upload's SVG check no longer refuses a `style` attribute outright"). `_cssUnescape()` decodes CSS escapes (`\` plus 1-6 hex digits, CSS Syntax Level 3 §4.3.7) before the same regex runs, closing the bypass without a second grammar to maintain — once the escape is gone, the literal match is exactly what a browser would have resolved.

- **Four residual gaps in `_sanitizeSvg()`, none of them fixed, all four now recorded as comments at the line a future reader meets them.** The 2026-09 review confirmed each survives the filter — an embedded control character folds `javascript:` past `/^javascript:/i` (`href="javas&#9;cript:alert(1)"`, XML attribute-value normalisation does not collapse the character reference the way HTML does, and a URL parser strips the TAB before resolving the scheme); a presentation attribute carrying the same `url()` a `style` value can (`fill="url(//evil.example/x.svg#g)"`) is checked nowhere, since only the `style` attribute ever runs through `_externalStyleRef`; `<!DOCTYPE svg SYSTEM "…">` survives `XMLSerializer` untouched, because no shipping engine resolves an external DTD to begin with; and an SVG event attribute prefixed to the SVG namespace itself (`s:onload`) still does not fire, because SVG defines its event handlers in no namespace at all, prefix or none. None is exploitable through how this bundle is actually served: the CGI answers a direct request with `Content-Security-Policy: default-src 'none'; sandbox`, and every other path renders the file as `mask-image`/`background-image`/`<img>`, which reads no `href`, executes no handler and resolves no DOCTYPE. Left unfixed on purpose — closing the first two costs either more string surgery over values a browser's own parser already handles better, or a `url()` check against every presentation attribute the SVG spec can grow, the same grammar-guessing this file exists to avoid, for a hole nothing reachable can use — but undocumented, each is a rediscovery waiting to be re-reported as new or "fixed" by someone who cannot see why it was left. Comment-only: costs nothing shipped, since terser strips comments before anything reaches flash (`resourcesJs` unchanged at 92,935 B).

### Performance

- **The CSS size budget rises to 127,500 B, 499 B of head-room, after the technical routes to stay under 127,000 B ran out.** `cascade.css` measured 127,001 B, 1 B over budget, once the day's seven RU/overflow fixes above (the toggle switch's knob, the form label, the interface-box head, the tab menu item, the Save & Apply footer, the row-action captions and the closed dropdown) were in — roughly 509 B of them, against 242 B won back the same day (dropping a scroller, -217 B; narrowing the row-action selector to `.cbi-section-actions .cbi-button`/`.btn`, -25 B) for a net cost of about 267 B. Two of the seven — the form label and the interface-box head — had left text completely unreachable, with no scroller anywhere near it, which is what buys the raise rather than a further trim. `tools/size-budget.mjs` carries the dated log.

## [0.14.11] — 2026-09-05

### Added

- **The content column's 1280px cap is now the reader's to move.** Appearance grows a Content width slider running from 1280px — the width every page in this theme was designed against — up to 3840px, which stands in for "uncapped": no CSS viewport in ordinary use reaches it, since even a physical 4K panel reports a scaled CSS width well under it. It only widens; there is no reason to offer a column narrower than the default, and keeping 1280 as the left end also puts `--fs-content-min` (500px, what the sidebar-to-bar fold is measured against) out of reach by construction rather than by a rule someone has to remember. The value is written as a real px length because `columnWidth()` reads the token through a `getComputedStyle` probe and `theme/50-toplayout.css` puts it inside a `calc()` — `none` and `100%` break both, and a value the CSS accepts but the probe cannot parse would leave the JS model saying 1280 while the box is wider. The modal follows the column deliberately, sharing the one token rather than keeping a second cap to reconcile. The fit re-runs on every move rather than deciding against the width the column had a moment ago — measured at a 1920px viewport with no reload, the real box and the model agree at every stop, off by 0. Asked for in #44.

- **A fifth colourway, `forum`, wearing the OpenWrt forum's own scheme.** Its colours are sampled from forum.openwrt.org rather than eyeballed — Discourse's `--primary`, `--secondary`, `--tertiary`, `--success`, `--danger` and `--highlight`, read in both of the site's schemes: white page with near-black ink and a `#0088cc` accent in light, `#222222` page with `#dddddd` ink and `#0f82af` in dark. None of the six clears 4.5:1 as *text* on the surface its own scheme pairs it with — Discourse checks contrast for a filled pill or an icon, not for the export tier every LuCI app prints text in — so each is moved the way `2020`'s cyan was: hue and saturation held, lightness walked until the ratio clears, with the before and after ratio recorded beside it (`#0088cc` 3.89:1 → `#0073ac` 5.19:1 on the page and 4.55:1 on a panel, `#009900` 3.78:1 → `#008000` 5.14:1, `#e45735` 3.68:1 → `#c63c1b` 5.17:1). `--highlight` has no footstrap analogue and its sampled `#ffff4d` measures 1.07:1 as text, so it fills the one status slot the forum's scheme has no colour for, `--fs-warn`. The masthead's navy stays out: `--secondary` is `#222` in both of the forum's schemes, so its page never turns navy, and the chrome reads `--fs-panel` regardless. The cascade budget goes to 129,700 B for it. In dark mode the bar wears the masthead's navy (`#002b49`, measured live) as a flat fill, like `--fs-bar-bg` everywhere else: the bar's ink measures 10.7:1 on it and its dimmed ink 5.8:1. Light mode keeps a white bar because that is what the forum's light scheme has (`--header_background #ffffff`, ink `#333333`).

- **A meter carries its own reading and colour.** `.cbi-progressbar` gains `role="progressbar"` plus `aria-valuemin`/`aria-valuemax`/`aria-valuenow` and an `aria-valuetext` spelling the whole reading ("11.98 GiB / 15.50 GiB (77%)"), plus a name taken from the row's own label where the markup carries one. Before this the reading lived ONLY in the `title` attribute, drawn on screen by `::after { content: attr(title) }` — generated content carries no text node, so the value did not exist for a screen reader at all (WCAG 4.1.2). Colour: `warn` from 80%, `danger` from 92% — `theme/25-progressbar.css` had never read `--fs-warn` or `--fs-danger` even once, and on a live Overview the same blue stood at Swap free 99%, Total Available 77%, Used 36% and Disk space 6%. The split is a decision, not a measurement (80% of an 84 MiB overlay and 80% of an 8 GiB disk are different news), chosen for predictability across every meter on the page. The annotation is not handed out through the one global `window.progressbar` alone: Overview's own includes (`20_memory.js`, `25_storage.js`, `30_network.js`) declare and call their OWN local `progressbar()`, never the theme's — so `fs-overview.js` instead walks every `.cbi-progressbar[title]` on each poll tick, inside the `arrange()` callback that already runs, with no second observer. A meter with no name in its markup names itself by its own reading: `aria-label=""` does not stop accessible-name computation — measured through CDP `Accessibility.getPartialAXTree`, Chromium falls back to `title` — and closing that is not reachable while the reading itself still lives in `title`, the only thing `::after` has to paint.

- **An Overview card collapses from the keyboard, and its own heading does it.** The page carried 14 `[data-clickable]` elements — the poll indicator plus one Hide/Show toggle per card — and every one was a bare `<span>`: no `tabindex`, no `role`, no `aria-expanded`, so Tab never reached one and a screen reader announced a run of text with no name, role or state (WCAG 2.1.1 and 4.1.2). The section heading is now the card's one focusable disclosure control (`role="button"`, `tabindex="0"`, `aria-expanded`, `aria-controls`, Enter and Space), while a mouse click still travels the exact path it did before: LuCI's own handler stays the only one, keyboard and heading forward into it, and nothing fires twice. The poll indicator in the header became a real named, keyboard-reachable button too, but carries no `aria-expanded` — it does not disclose anything, it pauses polling; the role goes only to an indicator carrying `data-clickable`, since `ui.showIndicator` sets that attribute only when it was handed a click handler, and a tabstop with no handler behind it would be worse than the bare span it replaces. The focus ring and pointer cursor are scoped the same way, to `[data-clickable]`, so the two cannot drift apart.

- **A `display` change on a table no longer erases its semantics.** LuCI's `.table` is a real `<table>`, and the theme changes its `display` in more than twenty places: on Overview that is Memory, Storage and the port list at every width, on a phone every `.fs-dt` (leases, leases6, assoclist and each app's own status table). Any `display` other than `table`/`table-row`/`table-cell`/… drops the implicit `table`, `row` and `cell` roles HTML-AAM derives from a real table (WCAG 1.3.1), and the column a value belongs to stopped being announced. `fs-select.js` now stamps the `table` / `rowgroup` / `row` / `columnheader` / `cell` chain unconditionally — unconditionally, because a media query's or a container's live state is not something JS can read at the point a role would need writing, and a role matching the implicit one costs nothing while the table IS a table, saving it the instant `display` moves away. `tools/table-contract.mjs` holds both sides of the pairing: it fails when the role map in the module shrinks, and it fails when `styles/**` gains a new table `display` rule the map has no role for. The three size budgets follow the numbers up: `cascade.css` to 127,000 B (126,481 measured), the shipped JS to 94,000 B (93,761) and a cold page to 58,100 B (57,894) — the four modules that grew are `fs-overview.js` 3.0 → 4.1 KB, `menu-footstrap-common.js` 3.3 → 4.2 KB, `fs-select.js` 9.5 → 9.9 KB and `fs-chrome.js` 6.3 → 6.7 KB, three of them cold.

### Changed

- **The bar is a solid surface, and the theme ships no decorative shadow and no blur.** The sticky bar was 88% of the panel under a 12px `backdrop-filter`; cards, tables, tab strips, alerts, the `#view > h2` plate, the SSH-keys panel and the range thumb each carried a 1px drop shadow. Measured on a stand at the page most favourable to the pair — a pattern wallpaper at full strength, the bar over scrolled content, 1180x720 at 2x — turning the shadows off moved 2,171 pixels of 3.4 million (0.064%) and turning the blur off moved 1,346 (0.040%). What the translucency did buy was visible: the page showing through the chrome as a sharp edge rather than as glass. Both go, and with them the four overrides that existed to undo them (`04-nocolormix.css`, `95-a11y-media.css`, the print sheet, the sidebar's own `backdrop-filter: none`). Kept: the focus rings, the pressed-button inset, the selected-row rail, and `--fs-shadow-pop`, which is what separates a floating layer — menu, dropdown list, modal, search box — from the page under it. 1.3 KB off `cascade.css`, budget down to 125,700 B. `computed-diff` reports the change and nothing else: 23 `box-shadow` values to `none` and one bar background from `color(srgb 1 1 1 / 0.88)` to `rgb(255, 255, 255)`, in each mode; axe-core stays clean across all 30 palette x mode x tint combinations.

- **Three more effects measured invisible and removed: the emboss, the text emboss and the pop surface's translucency.** Measured the same way as the shadows above — a variant sheet toggled on a live stand through `document.adoptedStyleSheets`, across 13 scenes (light and dark, a pattern wallpaper at full strength, the login page, an open modal, an open dropdown, a hovered submenu, a focused field, a hovered Save & Apply, and the pages that carry `.ifacebox` and `.zonebadge`), with a `base`-against-`base` control reading 0 in every one of them. `--fs-emboss` was reaching 34 elements per page and moved **no pixel at all** in any scene; `--fs-text-emboss` reached 433 and peaked at 181 pixels of 4.1 million (0.004%); `--fs-glass` — the pop surface at 96% of the panel — peaked at 173 (0.004%) and is now the flat panel. Confirmed after the fact rather than only before: the one page with no live counters (Firewall) is byte-for-byte unchanged, and on the pages that do carry them the whole difference sits in the uptime and traffic digits. What stays, because the same sweep showed it: `--fs-shadow-pop` (14,131 px on an open dropdown — it is what separates a floating layer from the page), `--fs-scrim` (88,042 px under a modal), `--fs-hover-lift` (2,004 px), the focus rings (653 px, and an accessibility contract besides), the alert tints and the status fills. 0.4 KB off `cascade.css`, budget to 125,300 B.

- **A key/value pair reflows once its container gets tight.** The first column held `width: 40%; white-space: nowrap` at every width: at 390×844 that is a 130px label against a 194px value, Firmware Version wrapping to 5 lines, Kernel Version and Architecture to 2, with half the label column standing empty. Under `@container fs-content (max-width: 560px)` the label now sits above the value as the same eyebrow the theme already sets section headings in. The threshold sits on the container, not the viewport, because in the sidebar layout the column runs narrow at a wide viewport. This is a width win, not a height one: on the gallery specimen (a 340px `.cbi-map`) the card grew 44.9% taller (202.6 → 293.5px), but the value column grew 1.67× wider (183.6 → 306px), and a long value moved from two lines to one.

### Fixed

- **Three cuts that move no computed value, and one prefix kept on purpose.** Five palettes declared `--fs-accent-soft` at the same 10% the default already carries — removed, while the dark blocks at 14% and 15% genuinely differ and stayed; 44 six-digit hex colours were three-digit; and `-webkit-appearance` went, its unprefixed form shipping at Chrome 84 / Firefox 80 / Safari 15.4, all at or below the theme's floor. 401 B off the sheet, with `computed-diff` at 0 by construction. The `-webkit-mask-*` prefixes were checked in the same pass and KEPT: unprefixed masking ships only from Chrome 120 against a floor of 108, so dropping them would have taken the wallpaper pattern and the masked icons away from every Chrome below that — `docs/css.md` now carries a table of which prefix is alive for which version, with the date it was checked, because this is the kind of thing that gets removed from memory.

- **Two byte cuts that were also drift cuts.** Four components restated the focus ring instead of joining the text-input group's selector list — a pseudo-element, the legacy `.cbi-select` shell and two `:focus-within` containers — so nothing stopped someone changing the ring on `select` and leaving the checkbox behind; they now share one body for the plain state and one for the invalid state, 432 B off the sheet. The `:has()` compound in that list is wrapped in `:is()`: unwrapped, a `:has()` compound sharing a selector list with plain ones fails the WHOLE rule below the browser floor (`docs/css.md`), rather than losing just that selector. And string literals repeated inside one module became a module-level `const`, 320 B off the shipped JS. That second one is worth doing here and almost nowhere else: uhttpd serves these files with no compression, so identity bytes are wire bytes, and terser has no pass that folds duplicate literals — while the biggest repeats in the tree, the `require …` loader pragmas, are code and were left alone. Both budgets follow the numbers down. Two of those hoists had to come back: `tools/axes.mjs` reads the attribute names out of `stampDark()`'s source and `tools/page-modules.mjs` reads the `data-page` value out of `fs-overview.js`, so a hoisted literal is invisible to them and both gates reported a drift that did not exist — the literals are spelled out again with a line saying why, and `docs/development.md` gains the trap alongside two more from the same audits.

- **`docs/releasing.md` step 7 no longer describes a feed pipeline that finishes on its own.** Cutting 0.14.10 showed it does not: the update bot's pull request is authored by `app/github-actions`, so the repository's `fork-pr-contributor-approval` policy holds its `pull_request` check in `action_required` and the automerge waits on a run nobody approved; and once merged, `Publish` never starts, because it triggers on a push to `main` and a push made with `GITHUB_TOKEN` does not trigger workflows. The step now says to approve the held run and dispatch `publish.yml` by hand, and to confirm the result by reading the served index rather than the workflow log. Both halves are filed as owfeed-packages#53.

- **The select's chevron costs one declaration instead of ten.** It was an inline SVG data-URI, and a URI cannot read `var()` — the stroke colour had to be written into the string, so the whole declaration was repeated once per palette and once per mode: ten copies of one glyph, 2,166 B of the shipped sheet, and ten chances to recolour a palette and forget its chevron. Two gradient bands draw the same V and take their colour from the palette's own `--fs-dim` at paint time, so a new colourway gets a correct chevron for nothing. The round line cap is gone; at 12px it is not visible, which was checked side by side before the swap. The cascade budget follows the number down to 127,600 rather than banking the room.

- **Two gates that could not see what they were guarding.** `npm run smoke` evaluated `fs-appearance.js` but never rendered the page it builds, so a call to `axes.currentContentWidth()` against a module that exported no such name threw during render and took the whole Footstrap section out of System with every gate green; and `docs/gallery.html` carried no chrome bar, so `computed-diff` answered "0 differences" for a change that repainted the bar's buttons and poll pill — including a rule written against `.label`, which the indicator does not carry (`ui.js` emits a bare `<span data-indicator=…>`), and which therefore matched nothing at all. `tools/lib/export-contract.mjs` now holds every `axes.*` / `prefs.*` name the Appearance page calls against what its module actually exports, before the browser starts; the gallery gained the bar in the shape the templates emit. Both were proved by re-breaking what they missed: removing the export fails `smoke` and names the two functions, and pointing the pill back at `--fs-panel2` turns `computed-diff` from 0 to 4.

- **`install-check` stopped calling a correctly installed router "behind".** The gate reads "the version the feed serves" as the highest candidate `apk list` answers with, and since the theme was accepted into the official openwrt/luci feed there are two builds of one name: ours, and one luci.mk stamps from git's commit date (`26.246.70755~4fd72fd`). That stamp only grows with the calendar, so the highest candidate is always the foreign one and the gate failed on owrt2512 and owrtsnap while owrt2410 — whose 24.10 feed carries no theme — passed. The candidate list is now filtered by the same rule `install.sh` pins with, so the two cannot drift apart. The explanation sits outside the `docker exec … sh -c '…'` payload, because a `#` line inside those quotes is shipped to the container as code.
...
- **Two targets a finger could not catch grew to 24px.** WCAG 2.2 SC 2.5.8, measured on System → System at 390×844: the range slider (`input[type=range]`) was 258×6 — the whole control, not just the thumb — so dragging Rounding on a phone meant catching a 6px band; the version link on Appearance was 84×17. The slider's box and its paint are split apart: a transparent 24px box, with the visible 6px track drawn inside it by `::-webkit-slider-runnable-track` and `::-moz-range-track`. No visible pixel moved, and that cost the second half of the fix: a flat `height: 24px` pulled the row taller through the line box (`.cbi-value-field` +2.75px in Chromium, +2.5px in Firefox, the track sitting ~1.25px lower), so `vertical-align: middle` on `.cbi-range-slider` was added alongside the symmetric negative margin that grows the box without moving the track — after which `.cbi-value-field` matches the prior layout to 0.000px in both engines, and `.cbi-value` diverges by 0.187px in Chromium and 0.183px in Firefox, under a fifth of a CSS px. The version link got the same invisible overlay the toggle switch already carries — 88×25 with the row's text, size and colour unchanged. The toggle switch, the third target named in the same finding, was already closed: its `::before` gives it a 44×30 target.

- **An empty card with a dash instead of a heading no longer draws.** A stock Overview include with nothing to show this tick still renders a full `.cbi-section` — title "-", the poll indicator its only content — 72px tall on a live Overview, seventh on the page (a fixture proves no position: it renders first on the gallery). `fs-overview.js` marks it `.fs-ovl-empty` on the same pass that wires the disclosure headers, and `pages/20-overview.css` drops it with `display: none`.
...
- **The uploaded wallpaper pattern is no longer served as a document a browser will execute.** Reported on the OpenWrt forum (topic 251930) by KONG and confirmed here: `/etc/footstrap/pattern.svg` was symlinked into `/www`, uhttpd served it as `image/svg+xml` with no session required, and opening that URL ran whatever was inside it in the router's origin — a marker SVG set `document.title`, read `location.origin` as the router and reached `document.cookie`, before login as well. The theme's own rendering never did that (`15-wallpaper.css` paints the file through `mask-image`, or as a tiled background, and neither runs script), so the exposure was the direct URL. Upload needs the theme's ACL group, which is normally an admin who already has root — but that group can be given to a restricted account, and a compromised `luci-app-*` in an admin's session has the same reach. The symlink is gone; the file is served by a handler that sets `Content-Security-Policy: default-src 'none'; sandbox` and `X-Content-Type-Options: nosniff`, verified with the same malicious file: direct hit executes nothing, the mask still paints. The handler takes no request input at all — the path is a shell constant — and answers `Cache-Control: immutable`, since the `?v=` token already changes with every upload. `_svgObjection()` additionally refuses `<style>`, the `style` attribute and `data:`/`blob:`/`about:` references, and now says in its own comment what it is: a hint for the admin uploading, not a defence, because a `curl` POST to the same ACL path skips it.

## [0.14.10] — 2026-09-04

### Added

- **One word per state in the Defaults row — "default" is not one of them.** The three buttons are `Save to router`, `Reset to router` and `Reset to built-in`, under a `Saving` section with a `Saved look` caption. Before, "default" named both the router's stored look (`Save as default`) and the theme's built-in one (`Reset to default`), while the router's look also answered to "saved" (`Reset to saved`) — three names for two states, and the forum asked outright what the two resets differed by (topic 251930, post 92). A zero-height spacer keeps the destructive reset off the save button's elbow, since the labels now differ by one word; both resets still require a second click.

- **The Appearance page states where saving happens, at both places that question comes up.** Two lines above the controls read "Footstrap theme settings apply at once and are stored permanently in this browser" and "Save & Apply at the foot of the page belongs to the System form and does not store them"; a clause per Defaults-row button then says what `Save as default` stores on the router and which state each reset lands in. Both come from the same forum thread (topic 251930, posts 83 and 90) — the button clauses cover what a banner cannot: with all three buttons named "save"/"reset", nothing said that `Reset to saved` follows the router while `Reset to default` restores the theme's built-in look. 613 B of shipped JS, almost all of it the sentences themselves; `fs-appearance.js` is fetched only by the System page, so a cold page downloads none of it.

### Changed

- **`docs/development.md` now names three stand traps that read as theme faults.**
  - A question gated on `/proc/mtd` or the overlay mount line has no answer on a container stand and fails silently: `view/system/flash.js` reads both through `fs.trimmed()` — `L.resolveDefault(read, '')` — so a missing file degrades to an empty string and the page renders complete, one control short. Running the same page under `/luci-static/bootstrap` is the measurement that tells a stand artefact from a theme fault; the note carries a Playwright route that feeds the overlay line into the ubus reply.
  - More than about ten unthrottled ubus calls in flight exhaust the stand's uhttpd, after which every request reports HTTP status 0 — indistinguishable at a glance from the outage a probe is built to catch. The base rate tells them apart: a real rpcd window is ~10% of reloads and ≤55 ms, a self-inflicted one is nearly every sample and outlives the event; polling serially at the natural 42–85 ms settle time avoids it.
  - `owlab up` can fail to rebuild the full five-router set — the 24.10 leg's image build dies on opkg index drift (`opkg_install_pkg: Checksum or size mismatch for package bash`, then `target owrt2410: failed to solve: … exit code: 255`), and Docker cancels the ImmortalWrt and snapshot legs with it — so the running stands must not be taken down while `up` is broken this way, since there is no volume to recover them from. `owlab test --release <version> --install <artifact> --assert …` synthesises its own router and needs no rebuild; run from a scratch directory (`../tmp/owlab-test-pretag`) so its containers, named after the cwd, do not collide with the project's own.

- **The session now works as a crew: an Opus lead thread that delegates, and `developer`, `tester`, `security` and `researcher` subagents in `.claude/agents/`.** The heaviest session on record put 484 Bash results and 20 screenshots through one lead context across just 5 delegations, and that volume is what kept filling it. One writer at a time; a read-only tester on a different model than the writer, covering T0, both owlab stands and — on the maintainer's word for that task — the hardware router; at most two fix rounds, with a repeated blocking finding going to the human as a stall. `tools/bg-wait.sh` pairs with every detached run. In `.claude/settings.json`, the owlab commands moved from `ask` to `allow` and `dev-sync.sh`/`ssh` moved to `ask`; the branch guard and the comment/review fences are gone, since publishing without an explicit instruction is now a `CLAUDE.md` rule rather than a matcher. `CLAUDE.md` shrank from 206 to 168 lines by pointing at `docs/` instead of repeating it: the comment rules live in `docs/conventions.md`, the probe rig and hardware proof in `docs/development.md`, the protocol in `docs/crew.md`.

- **The crew's own tooling is fenced where the permission matcher cannot reach.** `Bash(tools/bg.sh:*)` was allow-listed, but the matcher reads only a command's first word — so `tools/bg.sh sh -c '… ssh …'`, the exact form every T2 gate uses, carried `ssh`, `dev-sync.sh` and `git push` straight past the `ask` rules guarding the hardware router and publishing.
  - `tools/bg.sh` now refuses `ssh`, `scp`, `sftp`, `rsync`, `dev-sync.sh`, `git push|commit|tag` and `gh pr|release|issue`, scanning whole words with globbing off — so `sshd`, a `-ssh`-suffixed path, or a literal `*` all read as text; all ten real T2 invocations in `docs/development.md` still pass. A pre-release pass found three ways past that scan: a backslash was folded to a separator instead of deleted, so `sh -c 's\sh -V'` scanned as two harmless words and then reassembled into a real `ssh` at execution (verified: it printed the OpenSSH banner); `ssh`/`scp`/`sftp`/`rsync` matched only the bare word, so `/usr/bin/ssh host …` and `./ssh host …` walked straight through; and the publish check matched a literal `' git push '` substring, so `sh -c '/usr/bin/git push origin main'` was never seen. The scan now deletes backslashes before folding punctuation, matches `*/ssh`-style path forms for all four names, and tracks the previous word's basename for `git`/`gh` subcommands — `sshd` and a `-ssh`-suffixed path still read as text, and it is a guardrail against laundering a command past a permission rule, not a sandbox.
  - It also writes `<run-id>.pid`, and `tools/bg-wait.sh` now stops on three conditions instead of one — the status file, that pid gone, or a 7200 s cap — since a run killed by SIGKILL or a sleeping machine never writes a status, and the old waiter hung forever, then woke an already-finished session with it.
  - `tester`, `security` and `researcher` carry `disallowedTools: Edit, Write, NotebookEdit`, making read-only a fence rather than a prompt sentence; `git stash|clean|restore|reset --hard|checkout --` and `git add -A|--all|.` moved to `ask`, since the loop owns the index between rounds and `/upstream-pr` needs all of them.
  - `permissions.additionalDirectories` now registers `../tmp`, without which the three `Read/Write/Edit(../tmp/**)` rules never applied and every card write prompted.
  - `precommit-gate.sh` now reads the `-a` flag only from the part of a command before its first quote (`git commit -m "add -a flag"` used to be compared against a dirty HEAD it did not hold) and stands aside with a warning when `jq` is missing, instead of erroring before every bash call.
  - `measure-run.mjs` now also counts `<session>/subagents/*.jsonl`: the 2026-09-03 trial reads 152k characters in the lead against 1.54M across 17 subagent transcripts — the half of the trade the lead's own number could not show.

### Fixed

- **The `fast-uri` override moved to `^3.1.6`, closing four high-severity advisories: host confusion from skipped IDN canonicalisation on scheme-relative references and from percent-encoded scheme normalisation, and SSRF from malformed IPv6 normalisation and from repeated hostname percent-decoding.** All four are fixed in 3.1.6; the lock now resolves 3.1.7. It reaches the tree only as a devDependency — `stylelint` → `table` → `ajv` → `fast-uri` — so the shipped package, which carries nothing from npm, was never exposed; `npm audit --omit=dev` reports none. Listed under Fixed rather than a `Security` section, since `[Unreleased]` carries none this release.

- **A checkbox with a tooltip no longer draws a second, dead toggle switch.** `ui.Checkbox.render()` appends a second label — `label.cbi-tooltip-container`, holding the hint icon — beside the switch label whenever `form.Flag.tooltip` is set, and the switch rules matched both: reported on Network → Firewall → Zone Settings' "IPv4 Masquerading" (luci-app-firewall's `masq`, forum topic 251930, post 118), where bootstrap draws a checkbox plus a warning glyph and footstrap drew a toggle plus a second, unresponsive grey pill. The pill/knob/hit-target rules in `theme/60-inputs.css` and the forced-colors border in `theme/95-a11y-media.css` now key on `.cbi-checkbox > label[for]`, which `render()` gives only to the switch label — the cheapest discriminator too, since the sibling-selector form of the same fix costs 100 B more of shipped CSS. The tooltip label falls back to a plain icon glyph with an 8px gap (`--fs-space-2`), 0.5px off the switch's own top edge — the gallery's new fixture measured both labels 40x22 and flush before this, one 16x21 with a real gap after. The cascade budget in `tools/size-budget.mjs` moved 128,000 → 128,512 B to fit it, its 11 B of head-room being short of every form of the fix.

- **`postinst`/`postrm` no longer call `rpcd reload`, closing a race that could permanently drop an unrelated rpcd plugin.** Measured on owrt2512: reload is a full re-exec that re-`dlopen()`s every plugin `.so`, and a file absent or mid-write at that instant is dropped for good — `mv file.so /tmp; rpcd reload` loses `file` from `ubus list`, and restoring the file alone does not bring it back, only a further reload/restart does. Losing a plugin this way is what silently drops System → Backup/Flash Firmware's "Reset to defaults" row (`view/system/flash.js` reads the `-32000 Object not found` answer as "no overlay" — forum topic 251930, posts 97/109).
  - The theme ships no rpcd plugin, only `acl.d/*.json`, which rpcd re-reads at login, not only at start (measured: no reload needed for a restored ACL to take effect on the next login).
  - Root's stock `read='*' write='*'` login makes the theme's ACL group moot for a root session regardless (measured `true` even against an invented object).
  - The one case a reload used to help — a session already logged in on a restricted rpcd login before install — is now handled by ending that session only when a fresh login would actually gain the scope: denied `uci`/`footstrap`/`write` right now, **and** its own username's rpcd login lists the theme's ACL group. Every root session, every session with the scope already, and every denied session whose login was never given the group (a second admin account, a monitoring script) is left running.
  - The same call is gone from `dev-sync.sh`'s post-copy step for the same reason, leaving only the LuCI cache removal there. `docs/package.md`.
...
- **A gate for the one fitter whose element sits above the reader.** `tools/fit-quiet.mjs` (`npm run quiet`, in `live`). `fitChrome()` strips the bar's layout classes to ask whether the menu fits a row, and for the widths where it does the bar answers a row shorter than it stands — 98px against 65px at 767px, 123px against 115px at 480px on owrt2512 — which is the height the rest of the page is laid out against for the length of the pass. The symptom does not reproduce headless: the pass is one synchronous task, no frame is painted inside it, and both engines restore the offset once the bar is back (measured at half the page and at its bottom, 390/480/767px, 0px lost either way). So the gate watches the cause — it hands the bar a `classList` of its own and reads the height back at the instant the classes come off, since a live `DOMTokenList` carries no pointer to its element and no observer runs inside that task.
...
- **A leftover floor is judged against what the empty box stands at.** `tools/floor-contract.mjs` reported any `min-height` still on an emptied box, and a box with no children still has its padding: /admin/network/network measures 34px with the floor and 34px without it, which holds nothing up. The gate now clears the floor, re-measures and reports the difference, so the finding is the blank page and not the box.
...
- **A gate that reads the poll floor's height back, and empties a box to watch the floor come off.** `tools/floor-contract.mjs` (`npm run floor`, in `live` and in CI's motion slice). Two questions no other gate asks, because both faults ship as the same symptom — blank page — and neither is visible in a file or on a page that is only loaded: every `data-fs-floor` box is stripped, re-measured with `offsetHeight` (what the pre-0.14.4 shape wrote) and the two must agree within 4px; then the tallest floor on the page is emptied and must be given back. A box the poll refilled or replaced in the meantime is not judged — the Overview rewrites whole sections, and a detached node keeps whatever inline style it died with. 33 floors on owrt2512, worst 2px.

### Changed

- **A column heading keeps the case the app wrote it in.** `.th` carried `text-transform: uppercase`, so LuCI's own `IPv4 address` printed as `IPV4 ADDRESS` — which reads as a typo in a heading rather than as a style, and was reported as one (issue #39). The heading is still a heading: the smaller size, the weight, the tracking and the faint ink all stay; only the transform is gone, in the four rules that carried it — `.cbi-section .table .th` and the title row in `theme/30-tables.css`, the card label mirrored into `theme/65-dropdown.css`, and a table inside an alert in `theme/35-alerts.css`. It also un-breaks a class of selector: LuCI fills `data-title` from the heading's `innerText`, which is RENDERED text, so the theme's own uppercase was rewriting the string the theme's own CSS matched on — `[data-title="Description"]` on the package manager matched zero elements on every router, in every language. The chrome's eyebrows are unchanged: a menu section label, a KPI caption and the search palette's group headings are the theme's own words, not an app's.

### Removed

- **The navigation cross-fade is gone, and a client navigation on WebKit is up to 3.5 s shorter for it.** `swapIn()` wrapped the router's commit in `document.startViewTransition()` in 0.14.4, on the reasoning that the commit is one synchronous DOM move and the API would therefore be animating a frame rather than a render. That holds for the update callback and not for the CAPTURE the engine takes before entering it, which a page cannot bound: measured at 390px, Overview → System on WebKit spent 2,143 ms in a single main-thread task with the transition and 213 ms without, and the swap landed at 3,728 ms against 206. Until it lands the reader is looking at the page they navigated away from, under the new URL — which is what "the section is not where I expect it, F5 fixes it" looks like from outside (issue #42; F5 is a full load and starts no transition). A 150 ms deadline was tried and does not hold, because `skipTransition()` has to be called from a timer and the capture is holding the thread that timer needs: it fired at 1,944 ms, after the callback it existed to pre-empt. Chromium captured the same navigations in 15-40 ms and paid none of this. The effect can come back when the capture can be measured before it is spent — `docs/spa-router.md`.

### Fixed

- **A tab the reader opens starts where the page starts.** The poll floor writes `min-height`, which beats the `height: 0` an inactive tab pane is collapsed with, so the pane the reader leaves keeps the height it had while open. 0.14.6 takes that floor off the moment the sweep sees the pane hidden — but a tab switch mutates no node (`ui.tabs` writes `data-tab-active` on the panes), so nothing woke the sweep: on a page that polls the blank stood for one `pollinterval`, the "it puts itself right after a few seconds" of the reports, and on a page that does not poll for the life of the page. Measured on 25.12 at 1440px: System → Startup held 2432px of floor on `Initscripts` with the document at 3304px and the "Local Startup" textarea 2716px down, which is what a missing textarea looks like (#75, and the same report against 0.14.4 on the forum); Network → Interfaces held 1299px until the next tick. A third MutationObserver, filtered to `data-tab-active`, runs the same sweep at the switch: 900px and 1348px, with no floor left standing.

- **A table the app scrolls itself is left as a table.** The fit pass cards a data table when it needs more width than it has been given, and inside a box the app scrolls that question has already been answered by somebody else: within a 598px box a wide table overflows by definition, so the remedy ended in a card stack however much room the page had around it. `luci-app-filemanager` parks its listing in `#file-list-container.resizeable` (`resize: both; overflow: auto`, no width of its own), and the listing came out as cards on a 1280px screen with 1224px of column beside it. `fit.inScroller()` walks from the table to the content root and answers whether any ancestor scrolls horizontally; the question is asked BEFORE the slot's remembered answer is applied, so a table carded by an older build is un-carded rather than carded again. The walk stops at `#view` and `#modal_overlay`, because the dialog's scroller is the theme's own and the scroll fallback a foreign table gets is on the table itself. Swept over all 95 menu pages of a stand at 390px: one table matches, the file manager's, so nothing that cards today stops carding. Carded on the 0.14.3 package too — a case the rule never covered rather than a regression.
...
- **The live baseline carries banip's log page, and the runbook says how to run the wide gates.** `/admin/services/banip/processing_log` reports three findings on owrt2410 — 33px of sideways document scroll at 320, and the chrome's geometry against its real box off by 936px at 320 and -176px at 768. They belong to the app's own log markup and to how long that log has grown on a stand that has been up for days, not to the theme: the same three, to the pixel, on the shipped 0.14.5 build and on v0.14.3 (`c3f71d7`), one build per version on the same stand. The signature carries no numbers, so signing it is safe. `docs/releasing.md` also stops asking for a command that cannot work: `npm run live -- --all --pages-all` joins six `npm run` calls with `&&`, and npm appends the arguments after the last of them, so the flags reached `anchor` alone while every gate before it ran its default set — each is called by hand now, and the release's own wide run is what found it.

### Fixed

- **A router that serves both http and https stops greeting a logged-in reader with the login form.** The login page hops from http to https when the router answers on a TLS port, so that the password does not go over the clear connection — and that hop is CROSS-SITE: schemeful same-site puts the two schemes in different sites, so the browser withholds LuCI's `SameSite=strict` session cookie on exactly that navigation. A reader already logged in over https therefore landed back on the login form with a 403 behind it, plus luci-base's own `RPCError -32002` in the console from the view that renders there. Measured on a stand serving both schemes: that URL answers 403 to the hop and 200 both to `curl` carrying the cookie and to the next navigation the browser makes. The hop now leaves a `#fs-https` fragment behind, and the landing page spends one same-site navigation asking again: 403 http → 403 https → 200 with the view and no console error. A reader with genuinely no session sees the login form once and stays on it — three documents, no loop — at the cost of one extra request. Stock LuCI never shows this because the same script upstream is dead code: `luci-base/ucode/template/sysauth.ut` splits `https_port` where it means `https_ports`, so its port list is empty and it never hops at all.
...
- **One page holds every mechanism that keeps the reader's place, and what each is worth.** `docs/anchoring.md` — the `ENGINE_ANCHORS` detection that decides which half runs, the floor that stops `dom.content()` shortening the document, the reference and its memo, the three corrections and the single write they share, and the guards that refuse to correct a page in motion. Each entry carries the measurement the sweep reports when that mechanism is disabled on its own: 568px and 610px without the floor, the full 120px without either correction, 185-520px of mid-flick jump without the guards on a moving page, -52px on five passes out of five without `putBack()` re-reading where the element landed. Two of the nine the sweep cannot reach at all, which the page says out loud — they are held by the measurement in their own comment, so CI does not catch a change there. A third, `settleDrift()`, is measurable only on `imm2410 @390 top large` and only across repeats (-60px on 2 passes of 7, against 0 of 6 with it), which is an argument against narrowing the sweep to the three core stands. Written because the machinery had grown to nine parts described in nine comments, and the question "is this one still earning its place" had no cheap answer.

### Fixed

- **A router that upgrades itself keeps its translation.** The catalogues became their own packages in 0.14.4, and `depends` points from a catalogue to the theme — so upgrading the THEME pulled in nothing, and a Russian router taking 0.14.4 through the feed lost every `.lmo` with the old package and went back to English with nothing saying why (issue #41). On 25.12+ the catalogue installs itself: `install-if: luci-theme-footstrap={version} luci-i18n-base-<lang>` — apk's own conditional, which fires once the theme is at that version and the router already carries that language's LuCI base, and re-fires on every theme upgrade because the pin moves. opkg has no conditional form of this (`Recommends:` is executed but unconditional, and a postinst cannot call opkg, which holds an exclusive lock), so 24.10 stays with `install.sh` — which now covers install, re-run and upgrade alike, taking the catalogue from the signed release when the feed has not got it yet. Measured end to end on both package managers: bare router, second run, upgrade from 0.14.3, and the feed upgrade that started this.

- **The Appearance tab names the translation package when the router has not got it.** One line under the version, on a non-English router whose theme strings come back untranslated — `_('Layout', 'footstrap')` is the test, a string every catalogue carries. It costs no request and no ubus call: a package manager cannot read `uci luci.main.lang`, and this is the one place that knows both facts at once.

- **A hidden tab pane stops holding the page open.** The poll floor writes `min-height` from what a container's content measures, and `visibility: hidden` — how an inactive tab pane is collapsed — leaves that content in the layout with rects of its own. So the floor asked a pane the reader cannot see how tall it would be, got its full height and pinned the collapse open: on Network → Interfaces the hidden `device` pane held 893px, and every tab but the first showed its content that far down the page (issue #41, reported against a third-party app and reproduced on the stock page). Introduced in 0.14.4 by the switch from clear-and-remeasure to measuring the content, which is the shape that cannot see a collapse; a box with no height of its own is now refused, which also answers correctly for a container a tick has just emptied. Bisected on the stand: 0px through `6e57a5d`, 893px from `dc69b80` on.

- **The anchor sweep stops firing a real poll tick into its own next measurement.** `Poll.start()` in luci-base sets the interval and then calls `step()` synchronously, so the HOLD case handing the poll back on its way out dropped a live tick into the start of the SWAP case — which then measured the content column's floor while that tick rewrote the sections under it. CI reported `the content column's floor is not holding the document up`, 120px, on webkit/owrt2410 @390 top compact; the same cell is green at v0.14.2, green on every build without that change and green again when it was reverted, on a theme all three share. HOLD and SWAP now leave the poll stopped and QUIET — the one case that wants ticks landing mid-flick — is what starts it, before it parks and a tick's worth of time before it flicks: the same synchronous `step()` otherwise lands inside the flick QUIET is timing, which CI reported as `the offset moved on its own mid-flick (worst 145.5px)` on firefox/owrt2410 @390 top compact. 367 runs across owrt2512, owrt2410 and owrtsnap on firefox and webkit, no findings.

- **The reader stays put through a correction the page could not finish.** `putBack()` — the one write either anchoring correction makes — updated `_restAt` and left `_rest` alone, on the reasoning that the element is back at the top it was remembered at. It is not when the write was clamped short, and the memo then names a position nothing can reach: measured on webkit/Overview at 390 wide in the top layout, `_rest` claiming 93.02 for an element standing at 105, with the next tick carrying the reader 12px off (52px at the density beside it). Where the element actually landed is re-read after the write, for the reference and for its section fallback; `_rest.at` is left alone, being read only on the path where the engine does no anchoring — measured, the sweep is green on all 18 cells of the failing axis without it. It surfaced now because WebKit 26 implements scroll anchoring — verified behaviourally, not by `CSS.supports`: it compensates 300px of growth and holds the element still — so `ENGINE_ANCHORS` is true there and the theme corrects from a memo kept across ticks instead of a reference taken fresh on every mutation.

- **The anchor sweep says what it found while it is still running, and can be pointed at one cell.** Findings print as `FINDING …` the moment they are made rather than in a summary tens of minutes later, `--bail` stops the run at the first one, and `--width` / `--layout` narrow the 144-cell matrix to the cell a finding names — a minute against a full sweep, which is what five fix attempts against the WebKit fault above each used to cost. CI passes none of the three: how wide a fault is is half of what the sweep says.

- **The installer takes a translation from the signed release when the feed has not got it yet.** `luci-i18n-footstrap-ru` and `-es` are new packages in 0.14.4, and a release reaches owfeed-packages through a pull request against that repository — so for as long as that took, every Russian router running the installer was told `No luci-i18n-footstrap-ru in the feed` while the signed asset for it sat in the release. `install_language()` now falls back to `install_from_release()`, the same verified chain (TLS, `usign -V` against the pinned release key over `manifest.txt`, then the manifest's own sha256) the theme itself takes on a router the feed cannot serve. The fallback is pinned to the tag the INSTALLED theme came from, not to `latest`: the feed trails the release by up to a day, and a catalogue knows only the strings of its own version, so a 0.14.4 catalogue over a 0.14.3 theme renders the difference in English with nothing reporting it. `install_from_release()` takes that base as its second argument rather than assigning the global, which `feed_lag_note()` reads afterwards.

- **The palette's recents list remembers a section, not only the page it sits on.** Taking "Footstrap" from the search palette left `System` in the recently-visited list and the section itself nowhere: `fs-recent` stored a menu path and resolved it against the menu index alone, while a section has no dispatcher node and therefore no path that is only its own. An entry is a KEY now — a page's key is still its path, and a row from a companion source carries its own, `admin/system/system#Footstrap` being the page plus the section's heading — resolved against the whole pool, sources included, so every row the palette can offer is a row it can recall. `menu-footstrap-common` exports `remember()` for a source that has to record its own row, keeping the 8-entry cap and the de-duplication in one implementation; `warmRecent()` prefetches the page half of a key, and warms a page carrying two remembered sections once.

- **The anchor gate's hold probe stops measuring the page by an element a poll tick has deleted.** `HOLD` ran with the router's poll live, on the reasoning that a probe which only INSERTS survives a tick landing beside it — and the pad does. The reference does not: a tick calls `dom.content()` on the section the reader happens to be parked over, so the element the probe measured the page by is gone before the second read. Measured on the tag build, `the reader's element was replaced mid-measurement` on webkit/Overview at 1440 wide, on owrt2512 and owrt2410 both, and on no other engine — WebKit is slow enough here to lose that race repeatably. The growth `HOLD` reports is synthetic either way, so a real tick underneath it added nothing but the chance of measuring nothing. `SWAP` already held the poll for a different reason (it performs a tick by hand); `QUIET` still runs with it live, which its own note now says.
...
- **A navigation now cross-fades instead of cutting.** The router's commit — `commitStage()`, one synchronous DOM move — is wrapped in `document.startViewTransition()`, so the browser animates a FRAME and not a render: wrapping the navigation itself would freeze the page for the whole chain (136-196 ms median, up to the 15 s render timeout) which is what the API's rendering suppression does while the update callback is pending. 140 ms against the UA's 250 ms, because the warm navigation this sits on measures 136-142 ms and a transition longer than the work it covers reads as lag. No `view-transition-name` is declared anywhere and that is the design: the staged render keeps a SECOND `#view` in the document, two elements under one name abort the transition, and a named element taller than the snapshot containing block is clipped in its own snapshot — which every long status page is. The root snapshot is viewport-sized and bar, rail and sidebar are identical in both halves, so what visibly cross-fades is the content column. `::view-transition` gets `pointer-events: none`, or the pseudo covering the viewport would eat every click for the length of the animation, and the scroll restore moved inside the callback, or a scroll landing after the snapshot slides the live page under a still image. Reduced motion starts no transition at all rather than zeroing its duration — the `*` rule in `95-a11y-media.css` cannot reach a pseudo tree, and skipping also saves the snapshot. Same-document view transitions are Baseline as of 2025-10-14 (Chrome 111, Safari 18, Firefox 144); below that the API is absent, the CSS parses to nothing and the swap stays instant.

- **An optional package can add rows to the search palette and modules to the chrome, and the theme names none of them.** Three seams, 646 B of flash and 219 B of it on every page. `header.ut` reads `footstrap.settings.plugin`, whitelists each entry to the shape of a LuCI module name and prints `window.__fsPlugins`; the chrome requires each name at the end of its init, after the router and the page modules, so a plugin that throws costs only itself. `fs-search` builds its result pool from its own menu index PLUS the functions on `window.__fsSearchSources` and ranks everything through one `search()` — a GLOBAL rather than an export, because the palette is fetched on the first gesture and most sessions never make one, so a package that had to `require` it to register would pull 4.5 KB onto every page for a list nobody opened. A row from such a source carries an `onTake()` the palette calls when it is chosen: a section's href can only reach the page it lives on, so the source is what opens the tab and scrolls to it. `fs-menutree` exports raw node presence beside its alias-resolving lookup, which is what lets a plugin gate an action on the menu node carrying the ACL group that action needs — an alias resolves elsewhere and would answer for a permission the session may not hold. The first consumer is `luci-app-footstrap-palette` (section search and a `:` command line, 12,970 B), and none of it is in the theme.

### Changed


- **The Overview's cards stop shouting and start ranking what is on them.** Four measurements were the wrong way round. A key/value row drew its NAME at weight 600 in `--fs-text` and the figure beside it at 11px in `--fs-dim` — heavier and brighter than the number it introduces, which on a four-meter Memory card is four bold lines competing with the four values they label; the name is now `--fs-dim` at normal weight and the figure takes the body size and ink. `theme/30-tables.css` ruled every key/value row at 1px, so System carried nine dividers down a card whose two columns are a name and its value: gone, keyed on the SHAPE of the table (no header row) rather than on the three cards the old grid named, so a third-party include of the same shape gets it too — while a DATA table keeps its rules, the row there being one record among many. `.ifacebox-head.active` filled a full-bleed bar with `--fs-accent` across the card, three at once on this page (IPv4 Upstream, radio0, radio1), each louder than the data under it: the state it encodes is "this interface is up", so it is said in `--fs-good` on the same `--fs-panel2` every other head carries — measured at 4.59:1 at its worst (footstrap/dark) across all four palettes in both modes, AA throughout. And `L.itemlist` wraps every caption in `<strong>`, leaving Protocol/Address/Gateway the boldest text on the card: same inversion, same answer. Data tables lost one step of row padding, which is 88px off the DHCP card and 32px off Wireless. Scoped to the Overview: the same interface box on Network → Interfaces sits among many, where the fill is doing real work telling them apart.

- **Two ways the stands lie about the theme are written down.** `docs/playground.html` draws Port status as STOCK, and it is the build's doing: the reskin is scoped to `.ifacebox:has(img[src*="/port_"])` and `devkit-build.mjs` inlines every asset as a `data:` URI, so the icon's `src` no longer contains `/port_` and not one of those rules matches. Reading that as a regression is the trap; `document.querySelectorAll('#view .ifacebox:has(img[src*="/port_"])').length` is 0 there and one per port on a router. And `RPC call to uci/get failed: Access denied` on arrival is LuCI's, not the theme's — it is thrown once, before the login form is submitted, and appears identically on a stand carrying no local change; split the error capture at the login to tell. Both are in `docs/development.md`, under "The stand's own traps".


- **The maintainer is `Ivan Kvashonkin <vizzlef@gmail.com>` everywhere.** The Makefile's `LUCI_MAINTAINER` and both `owfeed.yml` entries — the feed's own maintainer and the theme package's — carried a handle and an address the maintainer does not read, so every built package and every feed index did too. `SECURITY.md` already named the right address; the package metadata agrees with it now, and the two language packages were written with it from the start.

- **The translations ship as their own packages again, and the theme is 4,821 B smaller for it.** The catalogues rode inside `luci-theme-footstrap` since v0.12.x, so every router carried Russian and Spanish — 10,992 B of flash — including the majority reading neither. They are `luci-i18n-footstrap-ru` and `luci-i18n-footstrap-es` now, the names and paths luci.mk emits, which also puts them back on `footstrap.<lang>.lmo` and lets a router that still owns that file from the old language package upgrade into it rather than collide with it. `tools/stage.sh` builds a staging tree per language, including the `uci-defaults` line that registers the language in LuCI's own menu; `owfeed.yml` declares one package each; `npm run i18n-packages` fails the build when `po/`, that script and that list disagree, and `tools/check-packages.sh` asserts the built set. Measured on both stands: the theme upgrade takes the old `footstrap-theme.*.lmo` away, installing `luci-i18n-footstrap-ru` puts `footstrap.ru.lmo` down, `luci.languages.ru` appears and the chrome renders in Russian on 25.12/apk and 24.10/opkg alike.

  Half of what made them bundled had already expired — the fielded self-update script that mis-picked a multi-asset release with `head -1` (issue #6) is retired, and `install.sh` reads the signed manifest and matches the package name exactly. The other half, that two packages drift, is the trade every `luci-app-*` in the tree makes, and `update-po.sh --check` already fails a build whose catalogues are behind the source. `install.sh` now reads `luci.main.lang` after installing the theme and fetches the matching catalogue best-effort, so the split does not silently un-translate a router on the upgrade that introduces it.

- **The app icon is 192 square, not 512, and the package is 2,617 B smaller for it.** A PNG is the one file here gzip cannot touch — 3,960 B of the icon's 4,098 survived compression, 5.4% of the whole `.apk` for one raster — and the mark is a flat two-tone glyph, so the pixels above 192 carried no detail. 192 is the size Chrome requires before it offers the install prompt at all, and iOS scales whatever `apple-touch-icon` names down to its own 180. Quantised and repacked the way the package ships it: 4,098 B becomes 1,481 B. The only place the difference is visible is Android's PWA splash screen, which draws the icon larger than any of its other uses.

- **The attribution override in `.claude/settings.json` is a string rather than `false`.** Both `attribution.commit` and `attribution.pr` exist to keep an agent's trailers out of a commit message and a pull request body; the value is now the string form. `.githooks/commit-msg` strips those trailers whatever the client does, so this is the client-side half of a rule the hook already holds on its own.

- **The browser floor is written down and derived from the sheet.** Nothing stated which browsers the theme runs on while the stylesheet quietly moved the bar twice. `npm run css-floor` walks the built `cascade.css`, classifies every at-rule, pseudo, value function, unit and property as hard (the page breaks without it) or soft (the page is plainer), and computes the floor: **Chrome 108, Firefox 101, Safari 15.4**, set by `@layer`, `:is()`, `:focus-visible`, `svh`/`dvh`, `accent-color` and the logical properties. `:has()` (105/121/15.4), `color-mix()` (111/113/16.2) and `@container` (105/110/16.0) are all above it and all degrade. The numbers live once, in `docs/css.md` between two markers, and the gate fails if the doc and the sheet disagree; a CSS feature the sheet has never used fails until it is classified. The JS floor is lower and is not the constraint — `requestIdleCallback` and `CSS.supports` are feature-detected and nothing else is younger than `ResizeObserver`.

- **HTML parsed from a string is held by a lint rule instead of by review.** The theme builds its chrome with `E()` and writes into `innerHTML` in four places, all safe — three `= ''` clears and one icon concatenation — and nothing checked that a fifth would be. `fs/no-unsanitized-html` (`tools/lib/eslint-no-unsanitized.mjs`, no new dependency) accepts a sink fed by string literals, a `const` bound to one, and two named icon builders, and rejects everything else including `insertAdjacentHTML` and `document.write`. The two builders are separated by what they do with the argument: `svgIcon()` wraps markup and has its arguments checked, `iconSvg()`/`iconFor()` map a name onto a closed set and never reach the output.

- **`npm run acl` states the three rules behind most of LuCI's recent security advisories.** The ACL is unchanged and already satisfied them; the gate is what keeps it that way. No `file.exec` in a `read` grant (advanced-reboot, samba4 and dockerman are each that sentence), no interpreter as the program (`/bin/sh <script>` is `/bin/sh <anything>` to rpcd), and a whole command line rather than a bare binary — the two grants this package ships are `/bin/chmod 644 /etc/footstrap/…`, exact strings, and it reports them by count.

- **The repository states its own contract: `SECURITY.md`, `CONTRIBUTING.md`, `.editorconfig`.** A package that accepts uploads and holds an rpcd ACL had no private channel for a report; `SECURITY.md` names one, the surfaces worth looking at and what is out of scope. `CONTRIBUTING.md` is the short path into `docs/`. Beside them: the four inline pre-paint blocks in `partials/head.ut` are one element now — two in the file, the data blob and the code, which is the split the lint processor keys off and the number a CSP would need nonces for — and `probe-span.mjs`, a one-off Playwright probe that had been committed at the repository root with an absolute path to another machine, is gone. Its finding was already written into `theme/30-tables.css`.
...
- **The release's slowest gate went from 52 minutes to about two and a half, and measures more than it did.** `anchors` was the whole pipeline's critical path — 52m20s and 52m38s on two runs against `live`'s 17m26s on the same three routers — because it was the only live gate still walking the stands one after another, and because one job ran both engines in series. The stands now run at once, the way `live-audit` and `spa-parity` already did: every wait in this sweep is on an EVENT (the scroll landing, `fit.restAt()` reporting a reference at this offset) with a bounded fallback, so three routers answering together cannot move an answer — unlike `scroll-jank`, whose subject IS frame pacing and which stays serial. Measured on chromium across three stands: 468s parallel against roughly 17 minutes serial, the same 132 runs and no new findings. firefox and webkit then split into one CI job each, since they share nothing but the sweep they run.

- **The anchor sweep crosses the axes that were measured to change its answer, and 12 cells a router instead of 48.** The matrix had grown by adding axes rather than by measuring them. Density: across all 72 Overview cells of a full sweep, normal, compact and large reported the same two outcomes and nothing else — `anchor on` clamped 0-1px with the reader still, `anchor off` clamped 59-60px with the reader moved 60-61px — so three densities measured one thing three times. Width and layout: of `1440 side`, `1440 top`, `390 side` and `390 top`, only the first scrolls `#maincontent` and the other three scroll the window, so four combinations covered two mechanisms. What was added instead is a page: the sweep now opens Status → Overview (section bodies a poll refills), Network → DHCP (a table whose rows it replaces, 32 and 36 leases on the stands, so the cell repeats) and Status → Processes (one table, a direct child of `#view`, where the climb out used to land on the host and give up). Two candidates were measured and refused, and the reasons are in the file: Wireless scrolls 812px at 390 wide and 0 at 1440, so every cell of it skipped, and the realtime Connections page follows the conntrack table — the same cell reported 958px of scroll in one run and 400px in the next, and the block this sweep swaps there is not the block its poll replaces, which moved the reader 252px against a 120px pad while the block itself measured the same height before and after. `--full` crosses every dropped axis anyway, and CI runs that on a push and a tag, where being wrong about a narrowing is still caught before a release.

- **The live job is three jobs and no longer the release's critical path.** With `anchors` split off it stood at 1545s, of which 1370 are gates: spa-parity 448, live-audit 427, and the remaining five 495 together. Those are the three slices (`parity`, `audit`, `motion`), each booting its own routers, so the wall clock is the longest of them rather than their sum — about 11 minutes against 26. `fail-fast` is off, because a parity failure says nothing about whether the reader stays put, and `install-check` keeps a slice's end to itself: it replaces the build under test with the published release, which may not happen on a router another gate is still measuring. Where two gates share a slice they now share the page shapes they read as well: classifying is one load and a 1200ms settle for every page of the menu, and spa-parity and live-audit each did the whole walk. `FS_SHAPES` names a directory the first one writes and the second reads — measured on owrt2512, live-audit went from 421s to 303s. It is off unless that variable is set, and CI points it inside the runner's own temp: a shape describes the page as THIS build renders it, so a cache that outlived an install would answer for a theme that is no longer there.

- **`build` no longer waits for the linter, and the guarantee moved to where it belongs.** `needs: [check, lint]` put 107 seconds in front of the job every live gate is queued behind, for an answer none of them reads — nothing the linter says changes what gets packed. `release` names `lint` in its own `needs` now, so a red linter still publishes nothing; it just stops delaying the four jobs that had no reason to wait for it.

### Fixed

- **The anchor sweep stopped emptying a section's DESCRIPTION and calling it a poll.** `.cbi-section > div` matches `.cbi-section-descr` as readily as a section body, and on DHCP at 1440px with the top bar that description is the tallest block above the reader — so the probe collapsed static text that luci-base appends once when the section renders (`form.js`) and no tick ever replaces. It reported 121px of movement on firefox and webkit across two routers, with the floor on that block standing at 158px against its 203px height. The selector excludes it now, the way it already excluded `.fs-ovl` for the same reason: a probe that performs a mutation the page never performs measures the gate, not the theme. The full sweep is green on both engines afterwards, 64 runs each.

- **A page whose content sits in a single wrapper is no longer read as an empty page.** `tools/scroll-anchor.mjs` decided there was "nothing to grow" by counting `#view`'s children and requiring two, which is a stand-in for "did this page render" that gets the answer wrong: the realtime Connections page puts everything inside one `.cbi-map`, so 3219px of content read as empty and every cell of that shape skipped without a word. It asks whether `#view` has a first element child now; whether there is enough of it to scroll is the next check's question, and that one asks in pixels.
...
- **The live gates no longer reach the open internet, so somebody else's fetch cannot fail a release.** A gate measures what this theme renders; anything a page pulls from a third-party host is another package's code talking to another host, and it can only add noise. `luci-app-ssclash` asks GitHub for the latest mihomo release on two of its pages, and on a CI runner — whose egress IP is shared and rate-limited — that answers 403: nine console findings across three routers, on a release that had nothing to do with any of them, none of it reproducible off the runner. `stands.mjs` now seals every gate's browser context to the router's own origin (`sealToRouter`), so the request never leaves; `data:` and `blob:` still pass, being something the page built for itself. Blocking beats both alternatives: filtering the message afterwards leaves the gate hostage to the network's mood, and dropping ssclash from the stands would cost the one adversary whose lazily-inserted Ace stylesheets can invert the cascade layer order. An abort still writes to the console, in a different sentence per engine — `Failed to fetch` in Chromium, `Load failed` in WebKit, `NetworkError` in Firefox, wrapped by the app in its own language — so the noise filter matches the engine's words inside the line rather than the app's.

- **A detached run is paired with a waiter, or it is a run nobody reads.** `tools/bg.sh` calls `setsid`, so nothing upstream knows the run exists: the log and a `.status` file are the whole interface, and `.status` appears only once the run is over. That makes "is it finished?" a question somebody has to keep asking, and therefore one that gets forgotten — three times in one session here, twice while the maintainer was waiting on exactly that answer, each time for 15-30 minutes after the chain had already ended. CLAUDE.md and `docs/development.md` now say to start a waiter on the `.status` file in the same turn as the run, so finishing wakes the reader instead of being polled for. "A detached run nobody opened is not a green gate" was already the rule; the waiter is what makes it practical to keep.

- **A synced stand served the modules the browser already had.** Every asset is served as `?v=<PKG_VERSION>-<pkgs_update_time>`, and that second half is the mtime of the package database (`runtime.uc:182`) — which `owlab sync` never touches, so a page ran a mixture: the module that was new got fetched, the one that only changed did not. It surfaced as `TypeError: search.pages is not a function`, thrown by a fresh module against a cached one from before its exports existed. `post_sync` touches both database paths now, which is the form `docs/development.md` already prescribed under "Caches while iterating" and nobody was running by hand.

- **The magnifier button and Ctrl+K stopped closing the palette once it had loaded.** Both the chrome's loader and the palette's own toggle were bound to the same button and the same two shortcuts, and after the first open both fired: the module's toggle closed the overlay and the loader re-opened it in the microtask after. Measured on a stand before the fix, the sequence `open, close, open` reads `open, open, open` for the button and the same for Ctrl+K; after it, `open, closed, open`. The loader stands down for the button, Ctrl+K and `/` as soon as the module is in hand, which is also the moment the module has bound its own.
...
- **A console line about a request that failed is no longer a finding.** The theme reaches no third-party host at run time — a rule of the package, not an accident — so a browser complaining that a resource would not load, or an app's own updater giving up on an HTTP status, cannot belong to it. `luci-app-ssclash` asks GitHub for the latest mihomo release on two of its pages, and on a GitHub Actions runner, whose egress IP is shared and rate-limited, that answers 403: nine findings across three routers, none reproducible off the runner, all of them its network rather than the page's markup. Dropped rather than baselined, because the text carries the stand's own port (`http://localhost:8024/…`) and such an entry goes stale the moment owlab hands out a different one. Everything that is not a network failure is still recorded — a TypeError out of a view, an unhandled rejection, a broken require of ours.

- **A table that grew PAST its parent was invisible to the fit engine, and the live gate saw it.** `fs-fit.overflows()` asked `scrollWidth > room`, which is the wrong question for a `display: table`: that box does not clip, it grows, so its scrollWidth and clientWidth rise together and stay equal to the room it was given. Measured on `#packages` at 1440: the table is 1226px wide inside a 1224px column, scrollWidth 1224, room 1224 — `overflows()` answered false and nothing carded. The comment above `roomFor()` describes exactly this trap, which is why the room comes from the parent; the comparison had stayed on the inside measurement. It takes the larger of the two now, so the theme asks the same question the gate does (`right > host + 1.5`). The table settles at 1224 without carding at all — the softer step, letting the widest column break, is enough — and the stacking thresholds do not move: still 820, with 860 now clean where it used to report 1px.

- **A live-audit overflow finding names the boxes instead of one number.** `table#packages (2)` says two pixels and nothing about WHICH box is two pixels too wide — the element, or a parent it fills at 100% — and that is a day of guessing on a finding that does not reproduce off the runner. It now reads `2px  el 1226 · host 1224 · parent div#view 1224 · border-box`, plus the scrollbar width when one was taking room, since a page long enough to scroll measures differently from the same page that is not. The first run with that line reproduced a CI-only finding locally and named the cause in one go. The signature the baseline is keyed on is unchanged.
...
- **38 tokens computed to `unset` below Chrome 111, Firefox 113 and Safari 16.2.** Every mixed token — the soft fills, the hairlines, the focus ring, `--fs-glass`, the export tier's `-medium`/`-low` — is a `color-mix()`, and a custom property keeps whatever token stream it is handed, so the failure lands at the point of use: `background: var(--fs-good-soft)` becomes invalid at computed-value time and computes to `initial`. That is not a fall back to the previous declaration, so the usual two-declaration trick does nothing. `styles/04-nocolormix.css` gives all 38 static twins under `@supports not (color: color-mix(…))`: tints degrade to `transparent` (an absent surface never lowers contrast), hairlines to the solid colour they are a fraction of, and the focus ring to `--fs-focus-ring-solo`, which was already mix-free — an invisible focus ring is an accessibility regression, not a cosmetic one. 1,276 B on the sheet.

- **A phone painted its address bar white over a dark page.** The theme shipped no `<meta name="theme-color">`, so the browser's own chrome — the address bar, the Android task-switcher card, an installed PWA's title bar — ignored every palette, mode and tint. `partials/head.ut` now ships one tag at the default palette's page colour, the same literal `manifest.json` carries for the same reason, and `fs-prefs.js` repaints it from the live body background through a single observer on `:root`, so it follows all 25 axes without a call in each applier. Read from the computed background, never from `--fs-bg`: a custom property hands back its token stream, and a mixed palette colour would land in the attribute verbatim.

- **The floor stopped being written where it cannot hold.** `min-height` is undefined on a table box (CSS 2.1 §10.7) and WebKit acts on it: a `.table.cbi-section-table` wearing a 313px floor still collapsed to 30px when its rows went, and the document lost 284px under the reader on `/admin/network/firewall` — the same on 24.10 and 25.12, while Chromium held the 313px. Writing the floor on the table's `.tbody` instead loses the same 284px, that box being a table box too. The floor now climbs to the nearest box that is not a table — the section — and the same emptied `.tbody` costs the document 0px on webkit and chromium alike, on both stands and on Firewall and DHCP. The three fixes together cost 353 B on a cold page and 305 B of flash, and the two budgets in `tools/size-budget.mjs` were raised by that much rather than the corrections being trimmed to fit.

- **The poll floor stopped telling the engine to drop its own anchoring twenty times a minute.** The floor is `min-height` on each container a poll empties, and it was re-measured by clearing every one of them, forcing a layout and writing the heights back. `min-height` is a scroll-anchoring suppression trigger on the path from the anchor to the scroller (css-scroll-anchoring-1 §3.2), so that shape asked the engine to stop compensating in the very frame the floor exists for. Measured over 25 s of real polling on the Overview at 390px: 1550 style writes on 25.12 and 170 on ImmortalWrt 24.10, of which 75 and 62 carried a value that had moved — the rest bought suppression for nothing. The height a container would stand at unaided is now read from the span of its children instead, so nothing is cleared and only a changed value is written: the same 25 s now cost 51 writes on 25.12 and 50 on 24.10, every one of them a real change. An empty container answers zero and keeps the floor it has, which is the case the floor was built for.

- **A poll tick on ImmortalWrt 24.10 left the page 58px out from under the reader: the theme's own put-off pass moved it, and nothing was looking.** A tick landing while the offset is in motion leaves its measurements to the pass that runs once the page holds still, and that pass re-lays the tables and rewrites every floor. An anchoring engine answers that layout change like any other — and `min-height` is itself a suppression trigger on the path to the anchor (css-scroll-anchoring-1), so the compensation can be off in the very frame that needs it. Measured on imm2410/WebKit: the pass's 88 `min-height` writes and a 58px jump of the offset land in the same frame, 429 ms after the mutation. `lateDrift()` could not see it, being scheduled from the mutation on the same `SCROLL_IDLE` — it read a drift of zero three milliseconds before the page moved, and `tools/scroll-anchor.mjs` reported the 58px on that cell alone out of 48 (@390, top layout, large density, Overview). The drift is now measured synchronously around that pass: the reader cannot scroll between two statements, so what it sees is the pass's own doing, and `getBoundingClientRect()` is the operation the spec makes an engine flush a pending adjustment before, so the read is a flush rather than a race. A first attempt that looked two frames later closed the cell but corrected inside a flick on three others; it does not ship.

  The comment beside the poll floor claimed a third selector — a table's `.tbody` — had been added and that its absence had cost a release. Neither held: `min-height` is undefined on a table box (CSS 2.1 §10.7) and WebKit acts on it, so a `.table.cbi-section-table` carrying a 313px floor still collapsed to 30px and the document lost 284px on `/admin/network/firewall` (24.10 and 25.12 alike; Chromium holds the 313px), and the same floor written on the `.tbody` loses the same 284px. 24.10 has no `.tbody` at all — its Overview renders `<table class="table">` with the rows directly inside. The comment states the measurement now.
...
- **The Appearance tab comes back 250 ms after a Save instead of up to five seconds later, because the observer that was supposed to notice had been watching a node the document no longer contained.** A client navigation builds a fresh `#view` before inserting it and swaps it in afterwards, while `watch()` runs on the `data-page` stamp — which comes first. So the node it bound was an orphan: measured after one round trip, `isConnected: false`, while `#view` and `#maincontent` were both alive. A Save's map redraw then produced four mutation batches that reached no callback at all, and the tab returned only when the five-second deadline fired. Binding to `#maincontent`, which outlives every swap, makes the swap itself a childList record: 48 checks across four stands, three engines, all at 250 ms.

  Two mechanisms then fell out on their own, which is the point of fixing a cause rather than padding a symptom. The retry ladder went first — instrumented, it mounted the tab zero times in ten sessions and only ever armed a timer that woke to find the work done. The deadline's repair attempt went with it: with the observer bound to a node that survives navigation the attempt is unreachable, and the whole matrix passes without it. The deadline stays as the question it was meant to be, naming the `ui.tabs` fact that must have moved if the tab is ever genuinely absent.

  What survived the same scrutiny is the `data-initialized` filter, and it survived by being proven rather than assumed: staging openwrt/luci#8903 — tear the tab, the pane and the attribute down in one task, restore the attribute alone 1,200 ms later — the tab is back 4 ms after the attribute, and with the filter removed it never comes back, on 25.12 and ImmortalWrt alike. No ordinary Save separates the two, which is why 48 of them found nothing. 141 B off `fs-appearance` after terser.
